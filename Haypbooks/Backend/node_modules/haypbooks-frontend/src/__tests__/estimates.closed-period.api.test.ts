...
import { POST as POST_ESTIMATES } from '@/app/api/estimates/route'
...
describe('Estimates closed-period enforcement', () => {
  test('blocks creating estimate dated on/before closed date', async () => {
    ...
    // Try to create estimate dated on closed date
    const resEst: any = await POST_ESTIMATES(req('http://localhost/api/estimates', 'POST', {
      number: 'EST-CP-1',
      customerId: 'c_1',
      date: closed,
      lines: [{ description: 'Scope', amount: 100 }],
      terms: 'Net 15'
    }))
    expect([400,403]).toContain(resEst.status)
