...
describe('Invoices closed-period enforcement', () => {
  test('blocks creating invoice dated on/before closed date', async () => {
    ...
    // Try to create invoice dated on closed date
    const resInv: any = await POST_INVOICES(req('http://localhost/api/invoices', 'POST', {
      number: 'INV-CP-1',
      customerId: 'c_1',
      date: closed,
      items: [{ description: 'Test', amount: 10 }]
    }))
    expect([400,403]).toContain(resInv.status)
