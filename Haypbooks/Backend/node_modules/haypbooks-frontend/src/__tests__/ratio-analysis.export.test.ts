...
describe('Ratio Analysis CSV export', () => {
  beforeAll(() => {
  ...
  it('returns CSV with As of caption and deterministic filename', async () => {
    const { GET } = await import('@/app/api/reports/ratio-analysis/export/route')
    const url = 'http://localhost/api/reports/ratio-analysis/export?period=Today'
    ...
    expect(disp).toContain('ratio-analysis-asof-2025-09-14')
