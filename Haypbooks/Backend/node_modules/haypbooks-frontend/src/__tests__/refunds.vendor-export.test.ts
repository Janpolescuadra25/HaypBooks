import { GET as GET_EXPORT } from '@/app/api/vendors/[id]/refunds/export/route'
import { seedIfNeeded, db } from '@/mock/db'

const makeReq = (url: string) => new Request(url)

describe('Vendor refunds CSV export', () => {
  test('caption, spacer, header; CSV-Version opt-in; filename tokens', async () => {
    try { seedIfNeeded() } catch {}
    const vendorId = db.vendors[0]?.id || 'ven_1'
    const base = `http://test/api/vendors/${vendorId}/refunds/export?asOf=2025-10-01`

    const resNo: any = await GET_EXPORT(makeReq(base), { params: { id: vendorId } } as any)
  ...
  expect(capNo).toContain('2025')
    ...
    const resYes: any = await GET_EXPORT(makeReq(base + '&csvVersion=1'), { params: { id: vendorId } } as any)
  ...
  expect(capYes).toContain('2025')
    ...
    expect(disp).toContain('vendor-refunds-asof-2025-10-01')
    expect(disp).toMatch(/ven-/)
  })
})
