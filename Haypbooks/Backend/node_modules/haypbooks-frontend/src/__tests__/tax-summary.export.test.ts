...
describe('Tax Summary CSV export', () => {
  beforeAll(() => {
  ...
  it('returns CSV with As of caption and deterministic filename', async () => {
    ...
    const url = 'http://localhost/api/reports/tax-summary/export?period=Today'
    ...
    expect(disp).toContain('tax-summary-asof-2025-09-14')
  ...
  it('omits CSV-Version by default and includes when opted-in', async () => {
    ...
    const urlNo = 'http://localhost/api/reports/tax-summary/export?period=Today'
    ...
    const urlYes = 'http://localhost/api/reports/tax-summary/export?period=Today&csvVersion=1'
  ...
  it('returns Forbidden when RBAC denies access', async () => {
    ...
    const { GET: GET_DENY } = await import('@/app/api/reports/tax-summary/export/route')
    const url = 'http://localhost/api/reports/tax-summary/export?period=Today'
    const res = await GET_DENY(new Request(url))
