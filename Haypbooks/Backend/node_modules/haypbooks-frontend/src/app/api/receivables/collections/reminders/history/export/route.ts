...
export async function GET(req: Request) {
  ...
  const data = await (jsonResp as any).json() as { start: string | null; end: string | null; history: { count: number; rows: Array<{ date: string; customer: string; invoice: string; batch?: boolean }> } }

  const lines: string[] = []
  if (versionFlag) lines.push('CSV-Version,1')
  lines.push(csvEscape(buildCsvCaption(data.start, data.end, data.end || todayIso())))
  lines.push(['Date','Customer','Invoice','Batch'].map(csvEscape).join(','))
  ...
  const filename = buildCsvFilename('collections-reminders-history', { start: data.start || undefined, end: data.end || undefined, asOfIso: data.end || todayIso() })
