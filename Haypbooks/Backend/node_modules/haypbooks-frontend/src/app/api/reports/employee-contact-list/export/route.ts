...
export async function GET(req: Request) {
  ...
  // JSON-first: delegate to sibling JSON route to avoid data drift
  ...
  const out: string[] = []
  // Optional CSV-Version prelude when explicitly requested
  if (parseCsvVersionFlag(url)) out.push('CSV-Version,1')
  ...
  const body = out.join('\n')
  const filename = buildCsvFilename('employee-contacts', { asOfIso: end })
