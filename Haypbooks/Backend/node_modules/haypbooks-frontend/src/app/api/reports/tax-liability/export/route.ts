...
export async function GET(req: Request) {
  ...
  const csvRows: string[][] = [header, [], ['Tax Agency', 'Rate Name', 'Rate %', 'Taxable Sales', 'Non-taxable Sales', 'Tax Amount']]
  if (versionFlag) csvRows.unshift(['CSV-Version','1'])
  for (const r of rows as Array<{ agency: string; rateName: string; ratePct: number; taxable: number; nontaxable: number; taxAmount: number }>) {
    csvRows.push([
      r.agency,
      r.rateName,
      String(r.ratePct),
      formatCurrency(Number(r.taxable || 0), baseCurrency),
      formatCurrency(Number(r.nontaxable || 0), baseCurrency),
  ...
  csvRows.push([
    ...
    formatCurrency(Number(totals?.nontaxable ?? 0), baseCurrency),
    formatCurrency(Number(totals?.taxAmount ?? 0), baseCurrency),
  ])

  const csv = toCSV(csvRows)
  const filename = buildCsvFilename('tax-liability', { asOfIso })
