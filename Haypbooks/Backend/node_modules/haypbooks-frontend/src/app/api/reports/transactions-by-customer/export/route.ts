...
export async function GET(req: Request) {
  ...
  const csvRows: string[][] = wantCsvVersion
    ? [["CSV-Version","1"], header, [], ['Date', 'Type', 'Number', 'Customer', 'Memo', 'Amount']]
    : [header, [], ['Date', 'Type', 'Number', 'Customer', 'Memo', 'Amount']]
  const baseCurrency = (db.settings?.baseCurrency as string) || 'USD'
  let grand = 0
  let current: string | null = null
  let subtotal = 0
  for (const r of (data.rows as Row[])) {
    if (current !== r.customer) {
  ...
  const filename = buildCsvFilename('transactions-by-customer', { asOfIso })
