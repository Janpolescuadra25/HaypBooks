...
export async function GET(req: Request) {
  ...
  // JSON-first: delegate to vendors JSON route (inherits vendors:read RBAC and filters)
  const jsonRes = await getJson(req)
  if (!('ok' in jsonRes) || !(jsonRes as any).ok) return jsonRes as any
  const data = await (jsonRes as any).json() as { vendors: Array<{ id: string; name: string }> }

  const lines: string[] = []
  if (versionFlag) lines.push('CSV-Version,1')
  lines.push(csvEscape(buildCsvCaption(null, null, end || todayIso())))
  lines.push(['Vendor ID','Vendor Name'].map(csvEscape).join(','))
  for (const v of data.vendors) {
    lines.push([v.id, v.name].map(csvEscape).join(','))
  }
  const tokens: string[] = []
  if (q) tokens.push(`q-${sanitizeToken(q)}`)
  const filename = buildCsvFilename('vendors', { asOfIso: end || todayIso(), tokens })
