...
async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/1099-transaction-detail-us${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function Page({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
  ...
  const asOfIso = (data?.end || (searchParams?.end ? new Date(searchParams.end + 'T00:00:00Z') : new Date()).toISOString()).slice(0,10)
  return (
    <div className="space-y-4">
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/1099-transaction-detail-us/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="1099 Transaction Detail">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">1099 Transaction Detail</div>
          ...
          <thead className="sticky top-0 bg-white/70 backdrop-blur z-10 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 text-center">Eligible (&gt;= $600 YTD)</th>
            </tr>
          </thead>
          ...
          <tfoot>
            <tr className="border-t border-slate-300 bg-white/60">
              <td className="px-3 py-2 font-medium text-slate-900 text-right" colSpan={3}>Total</td>
