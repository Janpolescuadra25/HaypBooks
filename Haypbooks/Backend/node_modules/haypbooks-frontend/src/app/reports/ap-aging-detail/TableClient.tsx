...
export default function APAgingTableClient({
  rows,
  totals,
  captionStart,
  captionEnd,
  ...
  const defaultVisible = COLUMNS.filter(c => c.required || ['number','dueDate','openBalance','vendor'].includes(c.key))
    .map(c => c.key)
  const [visible, setVisible] = usePersistedColumns('report:ap-aging-detail:columns', defaultVisible)
  ...
  return (
    <div className="glass-card overflow-x-auto">
      ...
      <table className="min-w-full text-sm caption-top" aria-label="A/P Aging Detail">
        <caption className="text-center py-3">
          <div className="text-base font-semibold text-slate-900">A/P Aging Detail</div>
          <div className="text-xs text-slate-600">{caption}</div>
          <ReportLiveClient>
            <>A P Aging Detail updated for {caption}. Total open balance <Amount value={Number(totals?.openBalance || 0)} />.</>
          </ReportLiveClient>
        </caption>
          ...
          <tr>
            {shown.map(col => (
              <th key={col.key} className={
                col.key === 'vendor' ? 'px-3 py-2 text-left' :
        ...
        <tbody>
          {rows.map((r, idx) => (
            <tr key={idx} className="border-t border-slate-200">
              {shown.map(col => {
                if (col.key === 'vendor') return <td key={col.key} className="px-3 py-2 text-slate-800">{r.vendor}</td>
                ...
                if (col.key === 'billDate') return <td key={col.key} className="px-3 py-2 text-center">{formatMMDDYYYY(r.billDate)}</td>
        ...
        <tfoot>
          <tr className="border-t border-slate-300 bg-white/60">
            {shown.map((col, i) => {
              ...
              // place Totals label under the first non-numeric column
