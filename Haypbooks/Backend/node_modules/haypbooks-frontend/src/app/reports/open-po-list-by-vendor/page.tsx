...
async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/open-po-list-by-vendor${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
      <div className="space-y-4">
        <div className="glass-card print:hidden"><BackButton ariaLabel="Back to Reports" /></div>
        <div className="glass-card">Access denied. You don’t have permission to view this report.</div>
      </div>
    )
  }
  return res.json()
...
export default async function OpenPOListByVendorPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/open-po-list-by-vendor/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Open Purchase Order List by Vendor">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Open Purchase Order List by Vendor</div>
          ...
          <tbody className="text-slate-800">
            {data.rows.map((r: any, idx: number) => (
              <tr key={idx} className="border-t border-slate-200">
                ...
                <td className="px-3 py-2 text-center tabular-nums"><Amount value={Number(r.amountOpen || 0)} /></td>
              </tr>
            ))}
          </tbody>
          <tfoot>
