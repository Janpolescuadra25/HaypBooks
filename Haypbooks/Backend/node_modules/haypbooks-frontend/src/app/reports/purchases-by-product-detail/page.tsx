...
async function fetchData(params?: { period?: string; start?: string; end?: string; item?: string; tag?: string }) {
  ...
  if (params?.tag) sp.set('tag', params.tag)
	...
	const res = await fetch(`${getBaseUrl()}/api/reports/purchases-by-product-detail${qs}`, { cache: 'no-store' })
	if (!res.ok) {
		return (
...
export default async function PurchasesByProductDetailPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string; item?: string; tag?: string } }) {
	const data = await fetchData({ period: searchParams?.period, start: searchParams?.start, end: searchParams?.end, item: searchParams?.item, tag: searchParams?.tag }) as any
	...
	return (
		<div className="space-y-4">
            ...
            {flags.tags && <TagSelect />}
						<RefreshButton />
						<ExportCsvButton exportPath="/api/reports/purchases-by-product-detail/export" />
						<PrintButton />
					</div>
				</div>
			</div>
      <ActiveFilterBar slug="purchases-by-product-detail" />
			<div className="glass-card overflow-x-auto avoid-break-inside">
				<table className="min-w-full text-sm caption-top" aria-label="Purchases by Product/Service Detail">
					<caption className="text-center py-3">
						<div className="text-base font-semibold text-slate-900">Purchases by Product/Service Detail</div>
