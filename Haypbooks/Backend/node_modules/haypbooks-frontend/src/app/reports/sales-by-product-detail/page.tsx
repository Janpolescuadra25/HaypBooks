...
async function fetchData(params?: { period?: string; start?: string; end?: string; product?: string; tag?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/sales-by-product-detail${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function SalesByProductDetailPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string; product?: string; tag?: string } }) {
  const data = await fetchData({ period: searchParams?.period, start: searchParams?.start, end: searchParams?.end, product: searchParams?.product, tag: searchParams?.tag }) as any
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/sales-by-product-detail/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <ActiveFilterBar slug="sales-by-product-detail" />
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Sales by Product/Service Detail">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Sales by Product/Service Detail</div>
          ...
          <thead className="sticky top-0 bg-white/70 backdrop-blur z-10 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 text-center">Product/Service</th>
          ...
          <tbody className="text-slate-800">
            {data.rows.map((r: any, idx: number) => (
              <tr key={idx} className="border-t border-slate-200">
                ...
                <td className="px-3 py-2 text-center">{r.product}</td>
                <td className="px-3 py-2 text-center">{r.customer}</td>
                <td className="px-3 py-2 text-center tabular-nums">{r.qty}</td>
                <td className="px-3 py-2 text-center tabular-nums"><Amount value={Number(r.rate || 0)} /></td>
