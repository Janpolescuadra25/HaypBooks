...
async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/tax-detail${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function TaxDetailPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/tax-detail/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Tax Detail">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Tax Detail</div>
            ...
            <div aria-live="polite" className="sr-only">
              Totals updated. Taxable {formatCurrency(Number(data.totals.taxable || 0))}, Tax amount {formatCurrency(Number(data.totals.taxAmount || 0))}.
            </div>
          </caption>
          <thead className="sticky top-0 bg-white/70 backdrop-blur z-10 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 tabular-nums">Taxable</th>
          ...
          <tbody className="text-slate-800">
            {data.rows.map((r: any, idx: number) => (
              <tr key={idx} className="border-top border-slate-200">
                ...
                <td className="px-3 py-2 text-left">{r.doc}</td>
