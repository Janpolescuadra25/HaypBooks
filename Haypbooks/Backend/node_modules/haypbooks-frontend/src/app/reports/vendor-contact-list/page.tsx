...
async function loadData(searchParams?: { q?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/vendor-contact-list${sp.size ? `?${sp.toString()}` : ''}`, { cache: 'no-store' })
  if (!res.ok) return null
  return res.json()
}

export default async function VendorContactListPage({ searchParams }: { searchParams: { q?: string } }) {
  const data = await loadData(searchParams)
  if (!data) {
    return (
      <div className="space-y-4">
  <ReportHeader exportPath="/api/reports/vendor-contact-list/export" />
  ...
  <ReportHeader exportPath="/api/reports/vendor-contact-list/export" />
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Vendor Contact List">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Vendor Contact List</div>
          </caption>
          ...
          <tbody className="text-slate-800">
            ...
            {data.rows.length === 0 && (
              <tr><td className="px-3 py-6 text-slate-500 text-center" colSpan={3}>No vendors.</td></tr>
            )}
          </tbody>
        </table>
      </div>
