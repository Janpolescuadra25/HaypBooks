-- Replace any RLS policies that reference tenantId_uuid_old to reference tenantId
DO $$
DECLARE
  r RECORD;
  new_qual TEXT;
...
BEGIN
  ...
  LOOP
    RAISE NOTICE 'Replacing policy % on %.%', r.polname, r.schema_name, r.table_name;
    -- Drop existing policy
    EXECUTE format('DROP POLICY IF EXISTS %I ON %I.%I', r.polname, r.schema_name, r.table_name);
    -- Replace tenantId_uuid_old with tenantId in expressions
    new_qual := replace(r.qual::text, 'tenantId_uuid_old', 'tenantId');
    new_with := replace(coalesce(r.withcheck, ''), 'tenantId_uuid_old', 'tenantId');
    -- Create the policy using the same name and replaced expressions. Use COALESCE to handle nulls.
