...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='"WorkspaceUser"' AND lower(column_name) = 'tenantid_old') THEN
  ...
  FOR r IN (SELECT conname, conrelid::regclass::text AS tbl FROM pg_constraint WHERE convalidated = false AND contype = 'f') LOOP
    BEGIN
      EXECUTE format('ALTER TABLE %s VALIDATE CONSTRAINT %I', r.tbl, r.conname);
      RAISE NOTICE 'Validated constraint % on table %', r.conname, r.tbl;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Could not validate constraint % on table %: %', r.conname, r.tbl, SQLERRM;
    END;
  END LOOP;
