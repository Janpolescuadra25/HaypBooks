...
;(async () => {
  ...
  try {
    ...
    const tenants = await prisma.workspace.findMany({ where: { createdAt: { gte: since } }, orderBy: { createdAt: 'asc' } })
    ...
    for (const t of tenants) {
      const existing = await prisma.company.findFirst({ where: { workspaceId: t.id } })
      if (existing) {
      ...
      const created = await prisma.company.create({ data: { workspaceId: t.id, name, isActive: true } })
      console.log('Created Company for tenant:', { tenantId: t.id, companyId: created.id, name: created.name })
      try {
        await prisma.workspace.update({ where: { id: t.id }, data: { companiesCreated: { increment: 1 } } })
      } catch (e) {
