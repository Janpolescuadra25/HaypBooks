...
(async ()=>{
  ...
  if (!conn) { console.error('Usage: node cleanup_workspaceuser_final_v3.js <connection-string>'); process.exit(2); }
  ...
  try {
    console.log('Beginning v3 cleanup transaction...');
    await c.query('BEGIN');
    ...
    // Drop any FKs referencing WorkspaceUser(tenantId_old, userId)
    ...
    // Drop indexes referencing tenantId_old on these tables before dropping columns
    console.log('Dropping indexes on WorkspaceUser/CompanyUser/PracticeUser that reference tenantId_old');
    ...
    console.log('Cleanup v3 committed successfully. Backups created:');
    backups.forEach(b => console.log('  -', b.b));
  } catch (err) {
    console.error('Error during cleanup v3, rolling back:', err.message);
