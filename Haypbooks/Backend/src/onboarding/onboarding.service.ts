        ...
        if (existingWorkspace) {
          ...
          try {
            ...
            if (mode === 'append' || !existingPractice) {
              resolvedPractice = await this.prisma.practice.create({ data: { name: normalizedFirm, workspaceId: existingWorkspace.id } })
              this.logger.log('[ONBOARDING-COMPLETE] ✅ Created new Practice record: ' + normalizedFirm)
            } else {
              resolvedPractice = await this.prisma.practice.update({ where: { id: existingPractice.id }, data: { name: normalizedFirm } })
          ...
          let newPractice: any = null
          try {
            newPractice = await this.prisma.practice.create({ data: { name: normalizedFirm, workspaceId: newPracticeWorkspace.id } })
