...
describe('Budgets (e2e)', () => {
  ...
  afterAll(async () => {
    await prisma.budgetLine.deleteMany({ where: { budget: { workspaceId } } })
  ...
  it('creates and retrieves budgets with lines', async () => {
    const budget = await prisma.budget.create({ data: { workspaceId, name: 'E2E Budget', scenario: 'BUDGET', status: 'DRAFT', fiscalYear: 2025, totalAmount: 50000 } })
