...
exports.seedDefaultRolesForTenant = seedDefaultRolesForTenant;
...
async function hasColumn(table, column) {
    const res = await prisma.$queryRaw `SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=${table} AND column_name=${column}) as exists`;
...
async function main() {
    ...
    const users = await prisma.$queryRaw `SELECT id, email, name, "isemailverified" as "isEmailVerified", "createdAt", "updatedAt" FROM public."User" WHERE email = ${'demo@haypbooks.test'} LIMIT 1;`;
    ...
    const tenants = await prisma.$queryRaw `SELECT id, "baseCurrency" FROM public."Tenant" WHERE CAST(id AS text) = ${DEMO_TENANT_ID} LIMIT 1;`;
    let tenant = tenants && tenants.length ? tenants[0] : undefined;
    if (!tenant) {
        ...
        const tenants2 = await prisma.$queryRaw `SELECT id, "baseCurrency" FROM public."Tenant" WHERE id = CAST(${DEMO_TENANT_ID} AS uuid) LIMIT 1;`;
    ...
    if (tenantUserHasStatus && tenantUserHasRoleId && tenantUserHasInvitedBy) {
        await prisma.workspaceUser.upsert({
            ...
            update: {},
            create: { workspaceId: tenant.id, userId: user.id, isOwner: true, lastAccessedAt: new Date() }
        });
    }
    else {
    ...
    if (companyHasTenantId) {
        try {
            ...
            try {
                const colType = await prisma.$queryRaw `SELECT data_type FROM information_schema.columns WHERE table_schema='public' AND table_name='Company' AND column_name='tenantId'`;
                const companyTenantIsUuid = colType && colType.length && colType[0].data_type === 'uuid';
                console.log('company.tenantId data_type', (colType && colType[0] && colType[0].data_type) || 'unknown');
                let exists = [];
                try {
                ...
                try {
                    const idType = await prisma.$queryRaw `SELECT data_type FROM information_schema.columns WHERE table_schema='public' AND table_name='Company' AND column_name='id'`;
                    const idIsUuid = idType && idType.length && idType[0].data_type === 'uuid';
                    if (idIsUuid) {
                        const rows = await prisma.$queryRaw `SELECT id, "tenantId", name FROM public."Company" WHERE id = CAST(${companyId} AS uuid) LIMIT 1;`;
                        demoCompany = rows && rows.length ? rows[0] : undefined;
                    }
                    else {
                        const rows = await prisma.$queryRawUnsafe(`SELECT id, "tenantId", name FROM public."Company" WHERE id = '${companyId}' LIMIT 1;`);
                        demoCompany = rows && rows.length ? rows[0] : undefined;
                    }
                }
                catch (err) {
        ...
        const rows = await prisma.$queryRaw `SELECT id, name FROM public."Company" WHERE id = ${'company-' + tenant.id} LIMIT 1;`;
        demoCompany = rows && rows.length ? rows[0] : undefined;
    }
    ...
    const customer = await prisma.customer.upsert({ where: { contactId: contact.id }, update: {}, create: { contactId: contact.id, workspaceId: tenant.id } });
    try {
    ...
    let invoice;
    try {
        invoice = await prisma.invoice.create({ data: { workspaceId: tenant.id, companyId: demoCompany.id, customerId: contact.id, invoiceNumber: 'INV-1001', date: new Date(), dueDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), totalAmount: 1000, balance: 1000, status: 'SENT' } });
        await prisma.invoiceLine.create({ data: { workspaceId: tenant.id, invoiceId: invoice.id, companyId: demoCompany.id, description: 'Consulting services', quantity: 1, unitPrice: 1000, totalPrice: 1000 } });
    ...
    try {
        ...
        if (taskTableExists) {
            ...
            if (!existing) {
                ...
                const companyColumnIsUuid = (await prisma.$queryRaw `SELECT data_type FROM information_schema.columns WHERE table_schema='public' AND table_name='Task' AND column_name='companyId'`);
                let companyIdToInsert = null;
                if (demoCompany?.id) {
                    const isUuidColumn = companyColumnIsUuid && companyColumnIsUuid.length && companyColumnIsUuid[0].data_type === 'uuid';
        ...
        console.warn('Skipping task seed; Task table may not exist yet', e);
    }
    // Create a sample CustomerCredit for demo (if table exists)
    try {
        await prisma.customerCredit.create({ data: { workspaceId: tenant.id, companyId: demoCompany.id, customerId: contact.id, creditNumber: 'CRED-1001', total: 100.00, balance: 100.00, status: 'ISSUED' } });
    ...
    let role = await prisma.role.findFirst({ where: { workspaceId: tenant.id, name: 'ADMIN' } }).catch(() => undefined);
    if (!role) {
        try {
            ...
            const roleColType = await prisma.$queryRaw `SELECT data_type FROM information_schema.columns WHERE table_schema='public' AND table_name='Role' AND column_name='tenantId'`;
            const roleTenantIsUuid = roleColType && roleColType.length && roleColType[0].data_type === 'uuid';
            let rows = [];
            if (roleTenantIsUuid) {
    ...
    let permission;
    if (permissionHasKey) {
        permission = await prisma.permission.upsert({ where: { key: 'manage:all' }, update: {}, create: { key: 'manage:all', desc: 'Full access for admins' } }).catch(() => undefined);
        ...
        const rows = await prisma.$queryRaw `SELECT key, desc FROM public."Permission" WHERE key = ${'manage:all'} LIMIT 1;`;
    ...
    if (role && role.id) {
        try {
            ...
            const tuColType = await prisma.$queryRaw `SELECT data_type FROM information_schema.columns WHERE table_schema='public' AND table_name='TenantUser' AND column_name='tenantId'`;
            const tuTenantIsUuid = tuColType && tuColType.length && tuColType[0].data_type === 'uuid';
            if (tuTenantIsUuid) {
    ...
    if (!paySchedule)
        paySchedule = await prisma.paySchedule.create({ data: { companyId: demoCompany.id, name: 'Weekly', frequency: 'WEEKLY' } });
    ...
    let project = await prisma.project.findFirst({ where: { workspaceId: tenant.id, name: 'Demo Project' } });
    if (!project)
    ...
    if (!demoTimesheet) {
        try {
            ...
            demoTimesheet = await prisma.timesheet.create({ data: { workspaceId: tenant.id, employeeId: employee.id, weekStart: new Date() } }).catch(() => undefined);
        }
    }
    ...
    if (!demoBudget) {
        try {
            ...
            demoBudget = await prisma.budget.create({ data: { workspaceId: tenant.id, name: 'Demo Operating Budget', scenario: 'BUDGET', fiscalYear: new Date().getFullYear(), totalAmount: 120000.00 } }).catch(() => undefined);
        }
    }
...
async function seedDefaultRolesForTenant(tenantId) {
    ...
    // Skip role creation when tenantId is not a valid UUID (tests/mocks use short ids like 't1')
    const isUuid = (s) => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s);
    if (!isUuid(tenantId)) {
        console.warn(`[seed] Skipping default roles creation for non-UUID tenantId: ${tenantId}`);
    }
    else {
        for (const template of roleTemplates) {
            // Check if role already exists
            let role = await prisma.role.findFirst({
                where: { workspaceId: tenantId, name: template.name }
            });
            if (!role) {
                try {
                    ...
                    try {
                        ...
                        const rows = await prisma.$queryRawUnsafe(`INSERT INTO public."Role" ("id","tenantId","name","tenantId_old") VALUES ($1::uuid,$2::uuid,$3,$4) RETURNING *`, roleId, tenantId, template.name, tenantId);
                        role = rows && rows.length ? rows[0] : null;
                        if (role)
                            console.log(`   ✓ Created role (raw): ${template.name}`);
                    }
                    catch (e2) {
