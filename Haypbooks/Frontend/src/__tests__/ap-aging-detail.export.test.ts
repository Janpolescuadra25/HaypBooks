import { GET as GET_EXPORT } from '@/app/api/reports/ap-aging-detail/export/route'

function makeReq(url: string): Request {
  return new Request(url)
}

describe('A/P Aging Detail CSV export', () => {
  test('includes date caption or range, correct header, and filename', async () => {
    const url = 'http://localhost/api/reports/ap-aging-detail/export?end=2025-09-04'
    ...
    expect(lines[2]).toBe('Vendor,Type,Number,Bill Date,Due Date,Aging (days),Open Balance')

    // Filename format suffix with as-of date
    expect(disp).toContain('ap-aging-detail-asof-2025-09-04')
  })
})
