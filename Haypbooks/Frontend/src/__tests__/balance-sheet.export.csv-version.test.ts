...
describe('Balance Sheet CSV-Version opt-in', () => {
  it('omits CSV-Version by default; caption is first line', async () => {
    const jsonRes: any = await BS_JSON(makeReq('http://test/api/reports/balance-sheet'))
    expect(jsonRes.status).toBe(200)

    const csvRes: any = await BS_CSV(makeReq('http://test/api/reports/balance-sheet/export?period=YTD&end=2025-01-31'))
  ...
  it('includes CSV-Version,1 when opted-in', async () => {
    const csvRes: any = await BS_CSV(makeReq('http://test/api/reports/balance-sheet/export?period=YTD&end=2025-01-31&csvVersion=1'))
    expect(csvRes.status).toBe(200)
    const text = await csvRes.text()
