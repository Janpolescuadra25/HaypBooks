...
describe('Bank Transactions triage: bulk exclude', () => {
  ...
  it('selects multiple rows and bulk excludes', async () => {
    ...
    expect(screen.getByRole('button', { name: /Exclude selected \(2\)/i })).toBeInTheDocument()

    await withinAct(async () => { await user.click(screen.getByRole('button', { name: /Exclude selected/i })) })
    await flushAsync()
    await waitFor(() => expect(screen.queryByRole('button', { name: /Exclude selected/i })).not.toBeInTheDocument())

    await withinAct(async () => { await user.click(screen.getByRole('tab', { name: /Excluded/i })) })
