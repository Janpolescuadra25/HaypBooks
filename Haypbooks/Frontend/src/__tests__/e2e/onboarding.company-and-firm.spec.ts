...
test('onboarding: owner companyName and accountant firmName persist via UI and backend', async ({ page, request }) => {
  ...
  // UI: ensure workspace page lists the new company name
  await page.goto('/workspace')
  await expect(page.locator(`text=${ownerCompany}`)).toBeVisible({ timeout: 10000 })

  // logout/sign-in again to confirm persistence
  ...
  // Clean up owner user if backend supports deletion[\s\S]*?await request.post('http://127.0.0.1:4000/api/test/delete-company'[\s\S]*?\}\).catch(() => null)
  ...
  // Accountant: complete onboarding via API since UI no longer exists
  await page.request.post('http://127.0.0.1:4000/api/onboarding/save', { data: { step: 'accountant_firm', data: { firmName: acctFirm } } })
  ...
  // UI: workspace table should show firm name (accountant practice)
  await page.goto('/workspace')
  await expect(page.locator(`text=${acctFirm}`)).toBeVisible({ timeout: 10000 })

  // sign out and sign back in to verify persistence
  await page.click('button[aria-label="Open user menu"]')
  await page.click('text=Sign out')
  await page.waitForURL('/login')
  // login again via API cookies for simplicity
  await page.context().addCookies([
    { name: 'token', value: loginJsonAcct.token, url: 'http://localhost', httpOnly: true },
    { name: 'refreshToken', value: loginJsonAcct.refreshToken, url: 'http://localhost', httpOnly: true },
    { name: 'email', value: loginJsonAcct.user.email, url: 'http://localhost' },
