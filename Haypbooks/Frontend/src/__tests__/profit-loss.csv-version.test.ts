import { GET as JSON_GET } from '@/app/api/reports/profit-loss/route'
import { GET as CSV_GET } from '@/app/api/reports/profit-loss/export/route'
...
describe('Profit & Loss CSV-Version opt-in', () => {
  ...
  test('omits CSV-Version by default and caption+header present', async () => {
    ...
    const jsonRes: any = await JSON_GET(makeReq('http://test/api/reports/profit-loss'))
    expect(jsonRes.status).toBe(200)
    const res: any = await CSV_GET(makeReq('http://test/api/reports/profit-loss/export'))
    expect(res.status).toBe(200)
    const lines = (await res.text()).split(/\r?\n/)
    expect(lines[0].startsWith('CSV-Version')).toBe(false)
    expect(lines[0]).toBeTruthy() // caption
    expect(lines[1]).toBe('Account,Current')
  })

  test('includes CSV-Version when flagged', async () => {
    setRoleOverride('admin' as any)
    const res: any = await CSV_GET(makeReq('http://test/api/reports/profit-loss/export?csvVersion=1'))
    expect(res.status).toBe(200)
    const lines = (await res.text()).split(/\r?\n/)
