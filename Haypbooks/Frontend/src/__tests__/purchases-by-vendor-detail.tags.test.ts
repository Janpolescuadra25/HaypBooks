...
describe('Purchases by Vendor Detail tag filter', () => {
  test('JSON and CSV honor tag when feature flag enabled', async () => {
    ...
    const baseRes: any = await PBVD_GET(makeReq('http://localhost/api/reports/purchases-by-vendor-detail?period=YTD'))
    const base = await baseRes.json()

    const taggedRes: any = await PBVD_GET(makeReq('http://localhost/api/reports/purchases-by-vendor-detail?period=YTD&tag=alpha'))
    ...
    const csvRes: any = await PBVD_EXP(makeReq('http://localhost/api/reports/purchases-by-vendor-detail/export?period=YTD&tag=alpha'))
