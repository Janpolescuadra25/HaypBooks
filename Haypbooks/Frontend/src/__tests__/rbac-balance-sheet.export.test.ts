...
describe('RBAC: Balance Sheet CSV export', () => {
  ...
  test('allows viewer and includes as-of token in filename', async () => {
    ...
    const res: any = await CSV_GET(makeReq('http://test/api/reports/balance-sheet/export?end=2025-09-04'))
    expect(res.status).toBe(200)
    const disp = res.headers.get('Content-Disposition') || ''
    expect(disp).toContain('balance-sheet-YTD-asof-2025-09-04')
  })
})
