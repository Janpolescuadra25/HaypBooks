...
describe('RBAC: Cash Flow CSV export', () => {
  ...
  test('allows viewer and includes as-of token in filename', async () => {
    ...
    const res: any = await CSV_GET(makeReq('http://test/api/reports/cash-flow/export?end=2025-09-04'))
    expect(res.status).toBe(200)
    const disp = res.headers.get('Content-Disposition') || ''
    expect(disp).toContain('cash-flow-YTD-asof-2025-09-04')
  })
})
