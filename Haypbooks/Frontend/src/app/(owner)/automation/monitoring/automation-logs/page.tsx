...
const FILTER_OPTIONS = ['All', 'Triggered', 'Completed', 'Failed', 'Skipped']
...
export default function AutomationLogsPage() {
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const response = await automationService.getAutomationLogs(companyId)
      setLogs(Array.isArray(response.data) ? response.data : response.data?.data ?? [])
      setFeedback(null)
    } catch (err: any) {
      setFeedback({ type: 'error', message: err?.message || 'Failed to load automation logs' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredLogs = filter === 'All' ? logs : logs.filter((item) => item.actionType === filter)
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-800">Automation Logs</h2>
            <p className="mt-1 text-sm text-slate-500">Monitor all automation activity and system events</p>
          </div>
        </div>
        ...
        <table className="w-full">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider px-4 py-3">WORKFLOW/RULE</th>
              <th className="text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider px-4 py-3">ACTION TYPE</th>
              ...
              <th className="text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider px-4 py-3">DETAILS</th>
            </tr>
          </thead>
              ...
              filteredLogs.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${ACTION_BADGE[item.actionType] || 'bg-slate-100 text-slate-600'}`}>
                      {item.actionType || '—'}
                    </span>
                  </td>
