...
export const MOCK_COA_ACCOUNTS: MockCOAAccount[] = [
  ...
  { id: 'acct-bank-charges',    code: '5300', name: 'Bank Charges Expense', type: 'Expense' },
  { id: 'acct-interest-income', code: '4100', name: 'Interest Income',      type: 'Revenue' },
];
...
// Mutable reconciliation history — seeded below and reset by resetMockState()
export let reconciliationHistory: MockReconciliation[] = [];
...
// ─── Pre-seed Reconciliation History (2 past reconciliations) ────────────────
;(() => {
  reconciliationHistory.push(
    {
      ...
      statementBalance: 125_000,
      calculatedBalance: 125_000,
      difference: 0,
      clearedTxIds: ['mt-023', 'mt-024'],
      outstandingTxIds: [],
      status: 'balanced',
    ...
    },
    {
      ...
      statementBalance: 175_000,
      calculatedBalance: 175_000,
      difference: 0,
      clearedTxIds: ['mt-011', 'mt-012', 'mt-013', 'mt-014', 'mt-015', 'mt-016', 'mt-017', 'mt-018'],
      outstandingTxIds: [],
      status: 'balanced',
      createdBy: 'Juan Dela Cruz',
  ...
  params.clearedTxIds.forEach(txId => {
    ...
    if (tx) {
      ...
      addAuditLog({
        ...
        entityId: txId,
        entityDescription: tx.description,
        details: 'Marked as reconciled during bank reconciliation',
      });
    }
  });

  // Auto-create service charge transaction if provided
  if (params.serviceCharge && params.serviceCharge > 0) {
    const svcId = `TX-SVC-${Date.now()}`;
    const svcTx: MockBankTransaction = {
      id: svcId,
      date: params.statementDate,
      description: 'Bank Service Charge',
      amount: -params.serviceCharge,
      accountId: params.bankAccountId,
      status: 'CATEGORIZED',
      transactionType: 'Bank Payment',
      accountCode: '5300',
      accountName: 'Bank Charges Expense',
      reconciled: true,
      manualEntry: true,
    };
    mockStore.items.push(svcTx);
    addAuditLog({
      action: 'manual_entry',
      entityType: 'transaction',
      entityId: svcId,
      entityDescription: 'Bank Service Charge',
      details: `Auto-created bank service charge of ₱${params.serviceCharge} during reconciliation`,
      newValue: 'Bank Charges Expense',
    });
  }

  // Auto-create interest income transaction if provided
  if (params.interestIncome && params.interestIncome > 0) {
    const intId = `TX-INT-${Date.now()}`;
    const intTx: MockBankTransaction = {
      id: intId,
      date: params.statementDate,
      description: 'Interest Income',
      amount: params.interestIncome,
      accountId: params.bankAccountId,
      status: 'CATEGORIZED',
      transactionType: 'Bank Receipt',
      accountCode: '4100',
    ...
    mockStore.items.push(intTx);
    addAuditLog({
      action: 'manual_entry',
      entityType: 'transaction',
      entityId: intId,
      entityDescription: 'Interest Income',
      details: `Auto-created interest income of ₱${params.interestIncome} during reconciliation`,
      newValue: 'Interest Income',
    });
  }

  const diff = Math.round((params.calculatedBalance - params.statementBalance) * 100) / 100;
  const record: MockReconciliation = {
    ...
    calculatedBalance: params.calculatedBalance,
    difference: diff,
    serviceCharge: params.serviceCharge,
    interestIncome: params.interestIncome,
    clearedTxIds: [...params.clearedTxIds],
    outstandingTxIds: [...params.outstandingTxIds],
    status: Math.abs(diff) < 0.005 ? 'balanced' : 'not_balanced',
    createdBy: 'Juan Dela Cruz',
...
export function getReconciliationHistory(bankAccountId?: string): MockReconciliation[] {
  return bankAccountId
    ? reconciliationHistory.filter(r => r.bankAccountId === bankAccountId)
    : [...reconciliationHistory];
}

export function getTransactionsForReconciliation(
  bankAccountId: string,
  endDate: string,
): MockBankTransaction[] {
  return mockStore.items.filter(
    tx => tx.accountId === bankAccountId && tx.date <= endDate,
  );
}
...
const INITIAL_MOCK_STATE = {
  ...
  reconciliationEntries: reconciliationHistory.map(r => ({
    ...r,
    clearedTxIds: [...r.clearedTxIds],
    outstandingTxIds: [...r.outstandingTxIds],
  })),
};
...
export function resetMockState(): void {
  ...
  reconciliationHistory.splice(
    0,
    reconciliationHistory.length,
    ...INITIAL_MOCK_STATE.reconciliationEntries.map(r => ({
      ...r,
      clearedTxIds: [...r.clearedTxIds],
      outstandingTxIds: [...r.outstandingTxIds],
    })),
