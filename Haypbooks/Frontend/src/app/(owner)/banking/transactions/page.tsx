    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={() => router.push('/banking/transactions/register')}
        ...
        return (
          <div className="bg-white border-b border-slate-200 shadow-sm px-6 py-2.5 flex items-center gap-3 flex-wrap">
            ...
            {selPendingIds.length >= 2 && (
              <button
                onClick={() => router.push(`/banking/transactions/match?selectedTxIds=${selPendingIds.join(',')}`)}
                        ...
                        {tx.status === 'MATCHED' && (
                          <div className="flex flex-col items-start gap-0.5">
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                onClick={() => router.push(`/banking/transactions/view-record?txnId=${tx.id}`)}
                              ...
                              {expandedTab === 'match' && (() => {
                                ...
                                return (
                                  <div id={`match-cards-${tx.id}`} className="max-w-2xl">
                                            ...
                                            return (
                                              <div key={je.id} className="px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-xs" onClick={e => e.stopPropagation()}>
                                                <div className="flex items-center gap-2 flex-wrap">
                                                  ...
                                                  <button
                                                    onClick={() => router.push(`/banking/transactions/view-record?type=${je.type === 'Bill' ? 'bill' : 'invoice'}&id=${je.id}&txnId=${je.id}`)}
                                    ...
                                    <button
                                      onClick={e => { e.stopPropagation(); router.push(`/banking/transactions/match?txnId=${tx.id}`) }}
                              ...
                              {expandedTab === 'split' && (
                                <div className="max-w-2xl">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={e => { e.stopPropagation(); router.push(`/banking/transactions/split?txnId=${tx.id}`) }}
                          ...
                          {tx.status === 'CATEGORIZED' && tx.transactionType === 'Split Transaction' && tx.splitLines && (
                            <div className="px-6 py-4 bg-slate-50 border-l-4 border-purple-400">
                              ...
                              <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                                <button onClick={() => router.push(`/banking/transactions/split?txnId=${tx.id}`)}
