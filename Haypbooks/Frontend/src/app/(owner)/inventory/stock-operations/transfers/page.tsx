...
import { RefreshCw, PackageOpen, ArrowLeftRight } from 'lucide-react'
...
export default function TransfersPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    setError('')
    try {
      const { data } = await inventoryService.listTransactions(companyId, { type: 'TRANSFER', limit: 999 })
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Track inventory transfers between locations</p>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400">
            ...
            <p className="text-sm">No stock transfers found.</p>
          </div>
        ) : (
