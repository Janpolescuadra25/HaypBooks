...
export default function SalaryStructuresPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true)
      setError('')
      const res = await payrollService.listSalaryStructures(companyId)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Salary Structures</h2>
        </div>
        <button
          type="button"
          onClick={() => fetchData()}
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-100">
            {data.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-sm text-slate-400">No salary structures found</td>
              </tr>
            ) : (
              data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-700">
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ${item.isDefault ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
