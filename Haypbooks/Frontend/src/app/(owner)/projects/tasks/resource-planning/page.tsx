...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await projectsService.listResourcePlans(companyId)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Resource Planning</h2>
        </div>
        <button
          type="button"
          onClick={() => fetchData()}
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full">
          <thead>
            <tr>
              <th className="text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider px-4 py-3">Resource</th>
              ...
              <th className="text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider px-4 py-3">Hours/Week</th>
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-3 text-sm text-slate-700">{item.resourceName ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{item.projectName ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{item.role ?? '—'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{item.hoursPerWeek ?? '—'}</td>
