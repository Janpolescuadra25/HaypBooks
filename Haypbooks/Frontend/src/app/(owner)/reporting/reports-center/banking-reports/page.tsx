...
export default function Page() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const response = await reportingService.getBankingReport(companyId, { from: dateFrom, to: dateTo })
      setData(Array.isArray(response.data) ? response.data : response.data?.data ?? [])
      setError('')
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || 'Failed to load banking report')
    } finally {
  ...
  const handleExport = async () => {
    if (!companyId) return
    try {
      const response = await reportingService.exportBankingReport(companyId, { from: dateFrom, to: dateTo })
      if (!response.ok) throw new Error('Failed to export report')
      await downloadFromResponse(response, 'banking-report.csv')
  ...
  if (companyIdError) {
    return (
      <div className="p-6 text-center text-red-600">{companyIdError}</div>
    )
  }
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-emerald-900">Banking Reports</h1>
            <p className="text-sm text-emerald-500 mt-1">Review bank activity, categories, and balances</p>
          </div>
        </div>
        <button
          type="button"
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={row.id ?? index} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                  ...
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.amount ?? 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.runningBalance ?? 0))}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
