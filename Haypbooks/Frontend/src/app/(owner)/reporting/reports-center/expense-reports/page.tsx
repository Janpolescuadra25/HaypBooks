...
export default function Page() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const response = await reportingService.getExpenseReport(companyId, { from: dateFrom, to: dateTo })
  ...
  const handleExport = async () => {
    if (!companyId) return
    try {
      const response = await reportingService.exportExpenseReport(companyId, { from: dateFrom, to: dateTo })
      if (!response.ok) throw new Error('Failed to export report')
      await downloadFromResponse(response, 'expense-report.csv')
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-500 mt-1">Track expense spend, vendors, and payment methods</p>
          </div>
        </div>
        <button
          type="button"
