...
export default function Page() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const response = await reportingService.getInventoryReport(companyId, { from: dateFrom, to: dateTo })
  ...
  const handleExport = async () => {
    if (!companyId) return
    try {
      const response = await reportingService.exportInventoryReport(companyId, { from: dateFrom, to: dateTo })
      if (!response.ok) throw new Error('Failed to export report')
      await downloadFromResponse(response, 'inventory-report.csv')
  ...
  const totalQuantity = data.reduce((sum, row) => sum + Number(row.quantityOnHand || 0), 0)
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-emerald-900">Inventory Reports</h1>
            <p className="text-sm text-emerald-500 mt-1">Monitor item stock, costs, and inventory value</p>
          </div>
        </div>
        <button
          type="button"
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={row.id ?? index} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                  ...
                  <td className="px-4 py-2 text-right">{Number(row.quantityOnHand ?? 0).toLocaleString()}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.unitCost ?? 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.totalValue ?? 0))}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-emerald-200 font-semibold">
                ...
                <td className="px-4 py-2 text-right">{formatAmount(totalValue)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
