...
export default function Page() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const response = await reportingService.getPayrollReport(companyId, { from: dateFrom, to: dateTo })
      setData(Array.isArray(response.data) ? response.data : response.data?.data ?? [])
      setError('')
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || 'Failed to load payroll report')
    } finally {
  ...
  const handleExport = async () => {
    if (!companyId) return
    try {
      const response = await reportingService.exportPayrollReport(companyId, { from: dateFrom, to: dateTo })
      if (!response.ok) throw new Error('Failed to export report')
      await downloadFromResponse(response, 'payroll-report.csv')
  ...
  const grossTotal = data.reduce((sum, row) => sum + Number(row.grossPay || 0), 0)
  const deductionTotal = data.reduce((sum, row) => sum + Number(row.deductions || 0), 0)
  const netTotal = data.reduce((sum, row) => sum + Number(row.netPay || 0), 0)
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-emerald-900">Payroll Reports</h1>
            <p className="text-sm text-emerald-500 mt-1">View payroll payouts, deductions, and net payroll details</p>
          </div>
        </div>
        <button
          type="button"
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={row.id ?? index} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                  ...
                  <td className="px-4 py-2">{row.payPeriod ?? '—'}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.grossPay || 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.deductions || 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.netPay || 0))}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-emerald-200 font-semibold">
                ...
                <td className="px-4 py-2 text-right">{formatAmount(grossTotal)}</td>
                <td className="px-4 py-2 text-right">{formatAmount(deductionTotal)}</td>
                <td className="px-4 py-2 text-right">{formatAmount(netTotal)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
