...
export default function Page() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      const response = await reportingService.getProjectReport(companyId, { from: dateFrom, to: dateTo })
      setData(Array.isArray(response.data) ? response.data : response.data?.data ?? [])
      setError('')
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || 'Failed to load project report')
    } finally {
  ...
  const handleExport = async () => {
    if (!companyId) return
    try {
      const response = await reportingService.exportProjectReport(companyId, { from: dateFrom, to: dateTo })
      if (!response.ok) throw new Error('Failed to export report')
      await downloadFromResponse(response, 'project-report.csv')
  ...
  const totalActual = data.reduce((sum, row) => sum + Number(row.actualCost || 0), 0)
  const totalVariance = data.reduce((sum, row) => sum + Number(row.variance || 0), 0)
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-emerald-900">Project Reports</h1>
            <p className="text-sm text-emerald-500 mt-1">Analyze project budgets, costs, and completion status</p>
          </div>
        </div>
        <button
          type="button"
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={row.id ?? index} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                  ...
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.budget || 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.actualCost || 0))}</td>
                  <td className="px-4 py-2 text-right">{formatAmount(Number(row.variance || 0))}</td>
                  <td className="px-4 py-2 text-right">{row.percentComplete != null ? `${Number(row.percentComplete).toFixed(0)}%` : '—'}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-emerald-200 font-semibold">
                ...
                <td className="px-4 py-2 text-right">{formatAmount(totalBudget)}</td>
                <td className="px-4 py-2 text-right">{formatAmount(totalActual)}</td>
                <td className="px-4 py-2 text-right">{formatAmount(totalVariance)}</td>
