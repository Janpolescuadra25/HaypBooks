...
export default function Page() {
  ...
  const [report, setReport] = useState<ARAgingResult | null>(null)
  ...
  const fetchReport = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const { data } = await apiClient.get(`/companies/${companyId}/ar/reports/aging`)
      setReport(data)
      setError('')
    } catch (e: any) {
      setError(e?.response?.data?.message ?? 'Failed to load A/R aging report')
    } finally {
      setLoading(false)
    }
  }, [companyId])
    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="border-l-4 border-emerald-500 pl-4">
          ...
          <h1 className="text-2xl font-black text-emerald-900 tracking-tight">A/R Aging Report</h1>
          <p className="text-sm text-slate-500">Outstanding receivables aged by overdue days.</p>
        </div>
        <button
          onClick={() => fetchReport()}
              ...
              <tbody>
                {report.customers.length === 0 ? (
                  ...
                  report.customers.map((c) => (
                    <tr key={c.customerId} className="border-b border-slate-50 hover:bg-emerald-50/40 transition-colors">
                      <td className="px-4 py-3 font-medium text-slate-800">{c.customerName}</td>
                      <td className="px-4 py-3 text-right text-emerald-700">{fmt(c.current)}</td>
                      <td className="px-4 py-3 text-right text-yellow-700">{fmt(c.days30)}</td>
                      <td className="px-4 py-3 text-right text-orange-600">{fmt(c.days60)}</td>
                      <td className="px-4 py-3 text-right text-rose-600">{fmt(c.days90)}</td>
                      <td className="px-4 py-3 text-right text-red-700">{fmt(c.over90)}</td>
                      <td className="px-4 py-3 text-right font-bold text-slate-800">{fmt(c.total)}</td>
                    </tr>
                  ))
                )}
              </tbody>
