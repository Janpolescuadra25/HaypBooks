...
export default function DelegatedTasksPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await tasksApprovalsService.listTasks(companyId, { assignedById: 'me' })
      const taskData = Array.isArray(response.data)
        ? response.data
        : response.data?.data ?? []
      setData(taskData)
    } catch (err: any) {
      setError(err?.message || 'Failed to load delegated tasks')
    } finally {
      setLoading(false)
    }
  }, [companyId])
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Delegated Tasks</h2>
        </div>
        <button
          type="button"
          title="Refresh"
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-700">{item.assignedToName || '—'}</td>
