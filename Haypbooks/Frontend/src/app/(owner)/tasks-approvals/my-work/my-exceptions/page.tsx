...
const STATUS_OPTIONS = ['open', 'in_progress', 'completed', 'on_hold', 'cancelled']
...
export default function MyExceptionsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await tasksApprovalsService.listTasks(companyId, { type: 'exception' })
  ...
  const filteredData = data.filter((item: any) => {
    ...
    const matchesSearch =
      ...
      item.exceptionType?.toLowerCase().includes(query) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">My Exceptions</h2>
        </div>
        <button
          type="button"
          title="Refresh"
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${statusFilter === '' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            All
          </button>
          {STATUS_OPTIONS.map((status) => {
            ...
            return (
              ...
              >
                {TASK_STATUS_LABELS[status]?.label || status}
              </button>
            )
          })}
        </div>
            ...
            {filteredData.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-sm text-slate-500">
                  {searchQuery || statusFilter
                    ...
                    : 'No exceptions requiring your attention.'}
                </td>
              </tr>
            ) : (
              filteredData.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-700">{formatExceptionType(item.exceptionType)}</td>
