    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Tax Returns</h2>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr>
              ...
              <th className="text-left px-4 py-3 text-[11px] font-medium uppercase tracking-wider text-slate-500">Filing Deadline</th>
              ...
              <th className="text-left px-4 py-3 text-[11px] font-medium uppercase tracking-wider text-slate-500">Payments</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center text-sm text-slate-400">No {statusFilter === 'All' ? 'tax returns' : `${statusFilter.toLowerCase()} returns`} found</td>
              </tr>
            ) : (
              returns.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">{item.payments?.length ? `${item.payments.length} payments — ${formatCurrency(totalPaid, currency)}` : '—'}</td>
                    <td className="px-4 py-3 text-slate-700 font-mono text-[11px]">{item.referenceNumber ?? '—'}</td>
                  </tr>
                )
              })
            )}
