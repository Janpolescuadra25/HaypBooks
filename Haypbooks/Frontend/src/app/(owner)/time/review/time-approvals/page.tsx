...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await timeService.listTimesheets(companyId, params)
  ...
  const handleApprove = async (timesheetId: string) => {
    ...
    try {
      ...
      await timeService.approveTimesheet(companyId, timesheetId)
  ...
  const handleReject = async (timesheetId: string) => {
    ...
    try {
      ...
      await timeService.rejectTimesheet(companyId, timesheetId, { reason: '' })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Time Approvals</h2>
        </div>
        <button
          type="button"
          title="Refresh"
        ...
        <table className="w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-3 text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider">Week Starting</th>
              ...
              <th className="px-4 py-3 text-left text-[11px] font-medium text-slate-500 uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {filteredData.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">No timesheets found</td>
              </tr>
            ) : (
