...
import { db, seedIfNeeded, listCustomerRefunds as dbListCustomerRefunds, createCustomerRefund as dbCreateCustomerRefund } from '@/mock/db'
...
export async function GET(req: NextRequest) {
  ...
  const refunds = dbListCustomerRefunds({ customerId }).map(r => ({
    id: r.id,
...
export async function POST(req: NextRequest) {
  ...
  try {
    const refund = dbCreateCustomerRefund({ customerId, amount, date, method, reference, creditMemoId })
