...
import { db, seedIfNeeded, listCustomerRefunds, createCustomerRefund } from '@/mock/db'
...
export async function GET(_req: Request, ctx: { params: { id: string } }) {
  ...
  if (!hasPermission(role, 'reports:read') && !hasPermission(role, 'invoices:read')) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }
  const id = ctx.params.id
  const rows = listCustomerRefunds({ customerId: id })
...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  ...
  const creditMemoId = body?.creditMemoId ? String(body.creditMemoId) : undefined
  ...
  try {
    const refund = createCustomerRefund({ customerId: id, amount, date, method, reference, creditMemoId })
