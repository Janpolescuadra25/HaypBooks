...
export async function GET(req: Request) {
  ...
  let rows = (json.rows as Array<{ method: string }>)
  ...
  out.push('')
  out.push(['Method'].join(','))
  for (const r of rows) out.push([r.method].join(','))

  const body = out.join('\n')
  return new NextResponse(body, {
    headers: {
      'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': `attachment; filename="${buildCsvFilename('payment-methods', { asOfIso: end })}"`,
