...
export async function GET(req: Request) {
  ...
  let rows = (json.rows as Array<{ name: string; type: string }> )
  ...
  out.push('')
  out.push(['Name','Type'].join(','))
  for (const r of rows) out.push([r.name, r.type].join(','))

  const body = out.join('\n')
  return new NextResponse(body, {
    headers: {
      'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': `attachment; filename="${buildCsvFilename('product-service-list', { asOfIso: end })}"`,
