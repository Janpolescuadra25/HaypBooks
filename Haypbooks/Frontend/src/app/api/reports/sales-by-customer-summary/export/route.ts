...
export async function GET(req: Request) {
  ...
  for (const r of rows as Array<{ customer: string; transactions: number; qty: number; amount: number }>) {
    csvRows.push([r.customer, String(r.transactions), String(r.qty), formatCurrency(Number(r.amount) || 0, baseCurrency)])
  }
  csvRows.push(['Totals', String(totals?.transactions ?? 0), String(totals?.qty ?? 0), formatCurrency(Number(totals?.amount ?? 0) || 0, baseCurrency)])

  if (versionFlag) csvRows.unshift(['CSV-Version','1'])

  const csv = toCSV(csvRows)
  const filename = buildCsvFilename('sales-by-customer-summary', { start: start || undefined, end: end || undefined, asOfIso })
