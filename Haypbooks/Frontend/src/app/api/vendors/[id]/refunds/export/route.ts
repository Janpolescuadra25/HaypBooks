...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  // JSON-first: delegate to sibling JSON route to inherit filters/RBAC/shape
  ...
  const data = await (jsonResp as any).json() as { refunds: Array<{ date: string; amount: number; method?: string; reference?: string; vendorCreditId?: string }> }
  ...
  for (const r of refunds) {
    rows.push([
      ...
      r.method || '',
      r.reference || '',
      r.vendorCreditId ? 'Credit' : '',
    ])
  }
  if (versionFlag) rows.unshift(['CSV-Version','1'])

  const csv = toCSV(rows)
  const filename = buildCsvFilename('vendor-refunds', { asOfIso, tokens: [`ven-${sanitizeToken(id)}`] })
