...
import { computeVendorStatement } from '@/mock/aggregations'
...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  const stmt = computeVendorStatement(id, asOf, { start, type })
  const vendor = db.vendors.find(v => v.id === id)
  return NextResponse.json({ ...stmt, vendor: { id, name: vendor?.name || id } })
}
