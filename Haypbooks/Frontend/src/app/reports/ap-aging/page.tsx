...
export default async function APAgingPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
  ...
  const fromUrl = new URL('https://app.local/reports/ap-aging')
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/ap-aging${sp.toString() ? `?${sp.toString()}` : ''}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
  ...
  return (
    <div className="space-y-4">
      <ReportHeader
        exportPath="/api/reports/ap-aging/export"
        periodValue={data.period}
      />
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Accounts Payable Aging">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">A/P Aging</div>
                ...
                <tr key={idx} className="border-t border-slate-200">
                  <td className="px-3 py-2 text-slate-800 text-center">
                    <a className="text-sky-700 hover:underline"
                      href={`/reports/ap-aging-detail${sp.toString() ? `?${sp.toString()}&vendor=${encodeURIComponent(r.name)}` : `?vendor=${encodeURIComponent(r.name)}`}&from=${encodeURIComponent(fromStr)}`}
                  ...
                  {buckets.map((b) => (
                    <td key={b} className="px-3 py-2 text-center tabular-nums">
                      <a
                        ...
                        href={`/reports/ap-aging-detail${sp.toString() ? `?${sp.toString()}&vendor=${encodeURIComponent(r.name)}&bucket=${encodeURIComponent(b)}` : `?vendor=${encodeURIComponent(r.name)}&bucket=${encodeURIComponent(b)}`}&from=${encodeURIComponent(fromStr)}`}
            ...
            <tr className="border-t border-slate-300 bg-white/60">
              ...
              {buckets.map((b) => (
                <td key={b} className="px-3 py-2 text-center tabular-nums">
                  <a
                    className="text-sky-700 hover:underline"
                    href={`/reports/ap-aging-detail${sp.toString() ? `?${sp.toString()}&bucket=${encodeURIComponent(b)}` : `?bucket=${encodeURIComponent(b)}`}&from=${encodeURIComponent(fromStr)}`}
                    aria-label={`View ${b.toUpperCase()} aging detail across all vendors`}
                  >
