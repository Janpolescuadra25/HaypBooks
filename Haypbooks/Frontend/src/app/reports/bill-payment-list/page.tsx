...
async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/bill-payment-list${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function BillPaymentListPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/bill-payment-list/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Bill Payment List">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Bill Payment List</div>
