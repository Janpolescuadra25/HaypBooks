...
type MonthlyLine = { name: string; values: number[] }

async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  const base = getBaseUrl()
  const u = new URL('/api/reports/profit-loss-by-month', base)
  ...
  return res.json() as Promise<{ period: string; start: string; end: string; months: string[]; lines: MonthlyLine[] }>
}
...
export default async function ProfitLossByMonthPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/profit-loss-by-month/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top bg-white border border-slate-200" aria-label="Profit and Loss by Month">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Profit &amp; Loss by Month</div>
          ...
          <thead className="sticky top-0 bg-white/70 backdrop-blur z-10 text-slate-600">
            <tr>
              ...
              {data.months.map((mk) => (
                <th key={mk} className="px-3 py-2 text-right font-semibold border-b border-slate-200">{monthLabel(mk)}</th>
              ))}
            ...
            {data.lines.map((l) => {
              ...
              return (
                <tr key={l.name}>
                  ...
                  {l.values.map((v, i) => (
                    <td key={i} className="px-3 py-2 text-right tabular-nums font-mono"><Amount value={v} /></td>
                  ))}
                  <td className="px-3 py-2 text-right font-semibold tabular-nums font-mono"><Amount value={total} /></td>
                </tr>
              )
            })}
          </tbody>
