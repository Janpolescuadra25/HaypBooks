...
async function fetchData(params?: { period?: string; start?: string; end?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/purchase-list${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function PurchaseListPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/purchase-list/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Purchase List">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Purchase List</div>
          ...
          <tfoot>
            <tr className="border-t border-slate-300 bg-white/60">
              <td className="px-3 py-2 font-medium text-slate-900 text-center" colSpan={6}>Totals</td>
