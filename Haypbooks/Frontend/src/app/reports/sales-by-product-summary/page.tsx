...
async function fetchData(params?: { period?: string; start?: string; end?: string; tag?: string }) {
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/sales-by-product-summary${qs}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
...
export default async function SalesByProductSummaryPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string; tag?: string } }) {
  ...
  const fromUrl = new URL('https://app.local/reports/sales-by-product-summary')
      ...
      <div className="glass-card print:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
            ...
            <RefreshButton />
            <ExportCsvButton exportPath="/api/reports/sales-by-product-summary/export" />
            <PrintButton />
          </div>
        </div>
      </div>
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Sales by Product/Service Summary">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Sales by Product/Service Summary</div>
              ...
              <tr key={idx} className="border-top border-slate-200">
                <td className="px-3 py-2 text-center">
                  <a
                    ...
                    href={`/reports/sales-by-product-detail?product=${encodeURIComponent(r.product)}${searchParams?.period ? `&period=${encodeURIComponent(searchParams.period)}` : ''}${searchParams?.start ? `&start=${encodeURIComponent(searchParams.start)}` : ''}${searchParams?.end ? `&end=${encodeURIComponent(searchParams.end)}` : ''}${searchParams?.tag ? `&tag=${encodeURIComponent(searchParams.tag)}` : ''}&from=${encodeURIComponent(fromStr)}`}
                  >
                    {r.product}
                  </a>
                </td>
