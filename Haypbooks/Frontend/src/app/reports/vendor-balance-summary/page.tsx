...
export default async function VendorBalanceSummaryPage({ searchParams }: { searchParams: { period?: string; start?: string; end?: string } }) {
  ...
  const fromUrl = new URL('https://app.local/reports/vendor-balance-summary')
  ...
  const res = await fetch(`${getBaseUrl()}/api/reports/vendor-balance-summary${sp.toString() ? `?${sp.toString()}` : ''}`, { cache: 'no-store' })
  if (!res.ok) {
    return (
  ...
  return (
    <div className="space-y-4">
      <ReportHeader exportPath="/api/reports/vendor-balance-summary/export" periodValue={data.period} />
      <div className="glass-card overflow-x-auto">
        <table className="min-w-full text-sm caption-top" aria-label="Vendor Balance Summary">
          <caption className="text-center py-3">
            <div className="text-base font-semibold text-slate-900">Vendor Balance Summary</div>
                ...
                <tr key={idx} className="border-t border-slate-200">
                  <td className="px-3 py-2 text-slate-800">
                    <Link
                      ...
                      href={( `/reports/vendor-balance-detail${sp.toString() ? `?${sp.toString()}&vendor=${encodeURIComponent(r.vendor)}` : `?vendor=${encodeURIComponent(r.vendor)}`}&from=${encodeURIComponent(fromStr)}` ) as any}
                      aria-label={`View balance detail for ${r.vendor}`}
                    >
                      {r.vendor}
                    </Link>
                  </td>
