...
export default function AutoApplyVendorCredits({ vendorId, canManage = true }: { vendorId: string; canManage?: boolean }) {
  const { formatCurrency } = useCurrency()
  const [preview, setPreview] = useState<{ allocations: number; totalApplied: number; rows: { billNumber: string; creditNumber: string; amount: number }[] } | null>(null)
  ...
  const run = useMemo(() => async (opts: { dryRun?: boolean }) => {
    ...
    setBusy(true)
    try {
      const res = await fetch(`/api/vendors/${encodeURIComponent(vendorId)}/credits/auto-apply`, {
        method: 'POST',
      ...
      const rows = (data.results || []).map((r: any) => ({ billNumber: r.billNumber, creditNumber: r.creditNumber, amount: Number(r.amount) }))
  ...
  useEffect(() => { setPreview(null); setDoneInfo(null); setError(null) }, [vendorId])
        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-white">
          ...
          {preview.rows.length > 0 && (
            <table className="min-w-full text-sm">
              <thead className="text-slate-600"><tr><th className="px-3 py-2 text-left">Bill</th><th className="px-3 py-2 text-left">Credit</th><th className="px-3 py-2 text-right">Amount</th></tr></thead>
              <tbody className="text-slate-800">
                {preview.rows.map((r, i) => (
                  <tr key={i} className="border-t border-slate-200"><td className="px-3 py-2">{r.billNumber}</td><td className="px-3 py-2">{r.creditNumber}</td><td className="px-3 py-2 text-right tabular-nums">{formatCurrency(r.amount)}</td></tr>
                ))}
              </tbody>
            </table>
          )}
