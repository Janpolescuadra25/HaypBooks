...
export default function InvoiceWriteoffButton({ id, defaultAmount }: { id: string; defaultAmount: number }) {
  ...
  const [expenseAcc, setExpenseAcc] = useState<string>('')
  ...
  useEffect(() => {
    ...
    async function loadAccounts() {
      try {
        ...
        const list = (j.accounts || []) as Array<{ id: string; number: string; name: string; type: string }>
        if (!alive) return
        setAccounts(list)
        const def = list.find(a => a.number === '6000' && a.type === 'Expense')
        if (def) setExpenseAcc(def.number)
        else {
          const firstExp = list.find(a => a.type === 'Expense')
          if (firstExp) setExpenseAcc(firstExp.number)
        }
      } catch {}
    }
    loadAccounts()
  ...
  async function submit() {
    ...
    if (!expenseAcc) { setError('Choose an expense account'); return }
    setBusy(true)
    setError(null)
    ...
    try {
      const res = await fetch(`/api/invoices/${encodeURIComponent(id)}/writeoff`, {
        ...
        body: JSON.stringify({ amount: amt, date: date || undefined, memo: memo || undefined, expenseAccountNumber: expenseAcc })
      })
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 flex flex-col text-sm gap-1">
                <label className="text-sm font-medium text-slate-700" htmlFor="wo-exp">Expense account</label>
                <select id="wo-exp" className="border rounded p-1" value={expenseAcc} onChange={e=>setExpenseAcc(e.target.value)}>
                  <option value="">Select…</option>
                  {expenseAccounts.map(a => (<option key={a.id} value={a.number}>{a.number} · {a.name}</option>))}
                </select>
              </div>
              <div className="md:col-span-2 flex flex-col text-sm gap-1">
