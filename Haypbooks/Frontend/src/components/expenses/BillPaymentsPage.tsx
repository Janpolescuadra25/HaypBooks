...
export default function BillPaymentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) { const q = search.toLowerCase(); list = list.filter(r => (r.paymentNumber ?? '').toLowerCase().includes(q) || (r.vendorName ?? '').toLowerCase().includes(q)) }
    ...
    })),
{
  ...
  const handleExportCSV = () => {
    ...
    csvDownload(`bill-payments-${new Date().toISOString().slice(0, 10)}.csv`,
      ...
      sorted.map(r => [r.paymentNumber ?? '', r.vendorName ?? '', r.date, r.method ?? '', r.status ?? '', String(r.amount)]))
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={fetchPayments} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-emerald-200 text-emerald-700 rounded-lg hover:bg-emerald-50"><RefreshCw size={14} /></button>
      ...
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Search payments..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }}
          ...
          <div style={{ position: 'fixed', top: mt, left: ml, zIndex: 9999 }} className="bg-white border border-gray-200 rounded-xl shadow-xl py-1 w-52">
            ...
            {row.status === 'PENDING' && (
              <>
                ...
                <MenuBtn icon={<Ban size={13} />} label="Void Payment" danger onClick={() => handleVoid(row.id)} />
              </>
            )}
          </div>
        )
