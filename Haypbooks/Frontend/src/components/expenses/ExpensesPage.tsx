...
import { Plus, Search, MoreVertical, Download, Filter, SlidersHorizontal, CheckSquare, Square, X, ArrowUpDown, Eye, Send, Check } from 'lucide-react'
...
interface Expense { id: string; expenseNumber?: string; submittedBy?: string; category?: string; date: string; status?: string; amount: number }
...
const DEFAULT_COLS: ColDef[] = [
  { key: 'expenseNumber', label: 'Expense #',    visible: true, width: 140 },
  { key: 'submittedBy',   label: 'Submitted By', visible: true, width: 180 },
  { key: 'category',      label: 'Category',     visible: true, width: 160 },
...
const STORAGE_KEY = 'expense-capture-cols-v4'
...
const SAMPLE: Expense[] = [
  { id: 'exp-001', expenseNumber: 'EXP-2026-001', submittedBy: 'Patricia Mendoza', category: 'Transportation',  date: '2026-04-08', status: 'SUBMITTED', amount: 2850 },
  { id: 'exp-002', expenseNumber: 'EXP-2026-002', submittedBy: 'Jerome Villanueva',category: 'Meals',           date: '2026-04-07', status: 'DRAFT',     amount: 1360 },
  { id: 'exp-003', expenseNumber: 'EXP-2026-003', submittedBy: 'Aira Cruz',        category: 'Office Supplies', date: '2026-04-05', status: 'APPROVED',  amount: 1890 },
  { id: 'exp-004', expenseNumber: 'EXP-2026-004', submittedBy: 'Marco Reyes',      category: 'Transportation',  date: '2026-04-01', status: 'REIMBURSED',amount: 3200 },
]

function compare(a: Expense, b: Expense, key: SortKey, dir: 'asc' | 'desc'): number {
...
export default function ExpensesPage() {
  ...
  const [rows]                          = useState<Expense[]>(SAMPLE)
  ...
  const filtered = useMemo(() => {
    ...
    if (search) { const q = search.toLowerCase(); list = list.filter(r => (r.expenseNumber ?? '').toLowerCase().includes(q) || (r.submittedBy ?? '').toLowerCase().includes(q) || (r.category ?? '').toLowerCase().includes(q)) }
  ...
  const handleExportCSV = () => { setShowExport(false); csvDownload(`expenses-${new Date().toISOString().slice(0,10)}.csv`,['Expense #','Submitted By','Category','Date','Status','Amount'],sorted.map(r=>[r.expenseNumber??'',r.submittedBy??'',r.category??'',r.date,r.status??'',String(r.amount)]));showToast('CSV exported') }
  const visibleCols = cols.filter(c => c.visible)

  const renderCell = (row: Expense, key: string) => {
    switch (key) {
      case 'expenseNumber': return <span className="font-semibold text-gray-800">{row.expenseNumber ?? '—'}</span>
      case 'submittedBy':   return <span className="text-gray-700">{row.submittedBy ?? '—'}</span>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-emerald-900">Expenses</h1><p className="text-sm text-emerald-600/70 mt-0.5">{`${sorted.length} expenses`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => showToast('Coming soon')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Plus size={15} /> New Expense</button>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-400" /><input type="text" placeholder="Search expenses..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} className="w-full pl-9 pr-3 py-2 text-sm border border-emerald-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
      ...
      {selected.size > 0 && (<div className="bg-emerald-600 text-white rounded-xl px-4 py-2.5 flex items-center gap-3"><CheckSquare size={16} /><span className="text-sm font-semibold">{selected.size} selected</span><div className="flex items-center gap-2 ml-auto"><button onClick={() => { csvDownload(`exp-sel.csv`,['Expense #','Submitted By','Category','Date','Status','Amount'],sorted.filter(r=>selected.has(r.id)).map(r=>[r.expenseNumber??'',r.submittedBy??'',r.category??'',r.date,r.status??'',String(r.amount)]));showToast('CSV exported')}} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-xs font-semibold"><Download size={12}/> Export</button><button onClick={()=>setSelected(new Set())} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg"><X size={14}/></button></div></div>)}
      ...
      {actionMenuId&&menuPos&&(()=>{const row=rows.find(r=>r.id===actionMenuId);if(!row)return null;const ml=Math.min(Math.max(4,menuPos.x-208),(typeof window!=='undefined'?window.innerWidth:800)-212);const mt=Math.min(menuPos.y+4,(typeof window!=='undefined'?window.innerHeight:600)-160);return(<div style={{position:'fixed',top:mt,left:ml,zIndex:9999}} className="bg-white border border-gray-200 rounded-xl shadow-xl py-1 w-52"><div className="px-3 py-1.5 border-b border-gray-100"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{row.expenseNumber??'Expense'}</p></div><MenuBtn icon={<Eye size={13}/>} label="View Expense" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>{row.status==='DRAFT'&&<MenuBtn icon={<Send size={13}/>} label="Submit" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>}{row.status==='SUBMITTED'&&<MenuBtn icon={<Check size={13}/>} label="Approve" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>}</div>)})()}
