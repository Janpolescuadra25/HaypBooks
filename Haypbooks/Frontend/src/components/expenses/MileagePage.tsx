...
const DEFAULT_COLS: ColDef[] = [
  ...
  { key: 'purpose',    label: 'Purpose',     visible: true, width: 180 },
  { key: 'route',      label: 'Route',       visible: true, width: 190 },
  { key: 'distanceKm', label: 'Km',          visible: true, width: 90,  align: 'right' },
  { key: 'rate',       label: 'Rate/Km',     visible: true, width: 100, align: 'right' },
  { key: 'amount',     label: 'Amount',      visible: true, width: 120, align: 'right' },
  { key: 'status',     label: 'Status',      visible: true, width: 130 },
]
const STORAGE_KEY = 'mileage-cols-v4'
...
const SAMPLE: MileageLog[] = [
  { id: 'ml-001', date: '2026-04-08', employee: 'Patricia Mendoza', purpose: 'Client Visit',     route: 'Makati → BGC',  distanceKm: 15, rate: 25, amount: 375,  status: 'SUBMITTED' },
  { id: 'ml-002', date: '2026-04-07', employee: 'Jerome Villanueva',purpose: 'Site Inspection', route: 'QC → Pasig',    distanceKm: 8,  rate: 25, amount: 200,  status: 'DRAFT'     },
  { id: 'ml-003', date: '2026-04-05', employee: 'Aira Cruz',        purpose: 'Supplier Meeting', route: 'Makati → Pasay', distanceKm: 12, rate: 25, amount: 300,  status: 'APPROVED' },
  { id: 'ml-004', date: '2026-04-02', employee: 'Marco Reyes',      purpose: 'Delivery',         route: 'BGC → Alabang', distanceKm: 25, rate: 25, amount: 625,  status: 'REJECTED' },
]

function compare(a: MileageLog, b: MileageLog, key: SortKey, dir: 'asc' | 'desc'): number {
  if (key === 'distanceKm') { const d = (a.distanceKm ?? 0) - (b.distanceKm ?? 0); return dir === 'asc' ? d : -d }
  if (key === 'rate')       { const d = (a.rate ?? 0) - (b.rate ?? 0); return dir === 'asc' ? d : -d }
...
export default function MileagePage() {
  ...
  const [rows]                          = useState<MileageLog[]>(SAMPLE)
  ...
  const filtered = useMemo(() => {
    ...
    if (search) { const q = search.toLowerCase(); list = list.filter(r => (r.employee ?? '').toLowerCase().includes(q) || (r.purpose ?? '').toLowerCase().includes(q)) }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-emerald-900">Mileage</h1><p className="text-sm text-emerald-600/70 mt-0.5">{`${sorted.length} logs`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => router.push('/expenses/expense-capture/mileage/new')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Plus size={15} /> Log Mileage</button>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-400" /><input type="text" placeholder="Search by employee or purpose..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} className="w-full pl-9 pr-3 py-2 text-sm border border-emerald-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
