...
import { Plus, Search, MoreVertical, Download, Filter, SlidersHorizontal, Clock, CheckSquare, Square, X, ArrowUpDown, Eye, Check, XCircle, RefreshCw } from 'lucide-react'
...
type ColDef = { key: string; label: string; visible: boolean; width: number; align?: EnhancedColumn<ProcurementApproval>['align'] }

const DEFAULT_COLS: ColDef[] = [
  { key: 'type',            label: 'Type',          visible: true, width: 100 },
  { key: 'referenceNumber', label: 'Reference #',   visible: true, width: 150 },
  { key: 'submittedBy',     label: 'Submitted By',  visible: true, width: 170 },
...
export default function ProcurementApprovalsPage() {
  ...
  const fetchRows = useCallback(() => {
    ...
    Promise.all([
      ...
      .catch(() => { setError('Failed to load approval items'); showToast('Failed to load approval items') })
  ...
  const handleColumnsChange = (next: EnhancedColumn<ProcurementApproval>[]) => {
  ...
  const table = useEnhancedTable<ProcurementApproval>({ data: paged, tableId: 'procurement-approvals' })
  const toggleSort   = (key: SortKey) => { if (sortKey === key) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey(key); setSortDir('asc') } }

  const columns: EnhancedColumn<ProcurementApproval>[] = [
    ...
    })),
{
  ...
  const renderCell = (row: ProcurementApproval, key: string) => {
    switch (key) {
      case 'type':            return <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${TYPE_STYLES[row.type ?? ''] ?? 'bg-gray-50 text-gray-600'}`}>{row.type ?? '—'}</span>
      case 'referenceNumber': return <span className="font-semibold text-gray-800">{row.referenceNumber ?? '—'}</span>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-emerald-900">Procurement Approvals</h1><p className="text-sm text-emerald-600/70 mt-0.5">{loading ? 'Loading...' : `${sorted.length} approvals`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => router.push('/expenses/procurement/approvals/activity')} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-emerald-200 text-emerald-700 rounded-lg hover:bg-emerald-50 transition-colors font-medium"><Clock size={15} /> Activity Log</button>
          <button onClick={() => showToast('Coming soon')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Plus size={15} /> New Approval</button>
        </div>
      </div>
      {error && <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</div>}
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-400" /><input type="text" placeholder="Search approvals..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} className="w-full pl-9 pr-3 py-2 text-sm border border-emerald-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
      ...
      {table.renderBulkToolbar([
        { label: 'Export Selected', onClick: () => table.exportSelectedToCsv(`procurement-approvals-selected-${new Date().toISOString().slice(0, 10)}.csv`, ['Type','Reference #','Submitted By','Date','Status','Amount'], (row) => [row.type ?? '', row.referenceNumber ?? '', row.submittedBy ?? '', row.date, row.status ?? '', String(row.amount)]) },
      ])}
      <EnhancedTable
        columns={columns}
        data={paged}
