...
export default function PurchaseRequestsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) { const q = search.toLowerCase(); list = list.filter(r => (r.prNumber ?? '').toLowerCase().includes(q) || (r.requestedBy ?? '').toLowerCase().includes(q) || (r.vendorName ?? '').toLowerCase().includes(q)) }
  ...
  const renderCell = (row: PurchaseRequest, key: string) => {
    switch (key) {
      ...
      case 'requestedBy': return <span className="text-gray-700">{row.requestedBy ?? '—'}</span>
      case 'vendorName':  return <span className="text-gray-600 truncate">{row.vendorName ?? '—'}</span>
      case 'date':        return <span className="text-gray-500">{fmtDate(row.date)}</span>
      case 'dateNeeded':  return <span className="text-gray-500">{row.dateNeeded ? fmtDate(row.dateNeeded) : '—'}</span>
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => router.push('/expenses/procurement/requests/activity')} className="flex items-center gap-1.5 px-3 py-2 text-sm border border-emerald-200 text-emerald-700 rounded-lg hover:bg-emerald-50 transition-colors font-medium"><Clock size={15} /> Activity Log</button>
      ...
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Search purchase requests..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} className="w-full pl-9 pr-3 py-2 text-sm border border-emerald-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          {STATUSES.map(s => (<button key={s} onClick={() => { setStatusFilter(s); setCurrentPage(1) }} className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${statusFilter === s ? 'bg-emerald-600 text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>{s === 'ALL' ? 'All' : s}</button>))}
        </div>
      ...
      {showAdvFilters && (
        <div className="bg-white rounded-xl border border-emerald-100 p-4 flex flex-wrap gap-4 items-end">
          <div><label className="block text-xs font-medium text-gray-500 mb-1">Date From</label><input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setCurrentPage(1) }} className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
          <div><label className="block text-xs font-medium text-gray-500 mb-1">Date To</label><input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setCurrentPage(1) }} className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
