...
import { Plus, Search, MoreVertical, Download, Filter, SlidersHorizontal, CheckSquare, Square, X, ArrowUpDown, Eye, Pause, Play, StopCircle } from 'lucide-react'
...
const SAMPLE: RecurringBill[] = [
  { id: 'rb-001', templateName: 'Monthly Office Rent',  vendorName: 'Luzon Properties Inc.',  frequency: 'Monthly',   nextDate: '2026-05-01', status: 'ACTIVE', amount: 45000 },
  { id: 'rb-002', templateName: 'Internet Service',     vendorName: 'Globe Telecom',          frequency: 'Monthly',   nextDate: '2026-05-10', status: 'ACTIVE', amount: 3500  },
  { id: 'rb-003', templateName: 'Quarterly Insurance',  vendorName: 'PhilCare Insurance',     frequency: 'Quarterly', nextDate: '2026-07-01', status: 'PAUSED', amount: 12000 },
  { id: 'rb-004', templateName: 'Annual Domain Renewal',vendorName: 'HostGator Philippines',  frequency: 'Annual',    nextDate: undefined,    status: 'ENDED',  amount: 8500  },
]
...
export default function RecurringBillsPage() {
  ...
  const [rows]                          = useState<RecurringBill[]>(SAMPLE)
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-emerald-900">Recurring Bills</h1><p className="text-sm text-emerald-600/70 mt-0.5">{`${sorted.length} templates`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => showToast('Coming soon')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Plus size={15} /> New Template</button>
        </div>
      </div>
      ...
      {selected.size > 0 && (<div className="bg-emerald-600 text-white rounded-xl px-4 py-2.5 flex items-center gap-3"><CheckSquare size={16} /><span className="text-sm font-semibold">{selected.size} selected</span><div className="flex items-center gap-2 ml-auto"><button onClick={() => { csvDownload(`rb-sel.csv`,['Template','Vendor','Frequency','Next Date','Status','Amount'],sorted.filter(r=>selected.has(r.id)).map(r=>[r.templateName??'',r.vendorName??'',r.frequency??'',r.nextDate??'',r.status??'',String(r.amount)]));showToast('CSV exported')}} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-xs font-semibold"><Download size={12}/> Export</button><button onClick={()=>setSelected(new Set())} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg"><X size={14}/></button></div></div>)}
