...
import { Plus, Search, MoreVertical, Download, Filter, SlidersHorizontal, CheckSquare, Square, X, ArrowUpDown, Eye, Check, Play } from 'lucide-react'
...
interface Reimbursement { id: string; reimbursementNumber?: string; employee?: string; dateSubmitted: string; totalAmount: number; status?: string }
...
const DEFAULT_COLS: ColDef[] = [
  { key: 'reimbursementNumber', label: 'Reimbursement #', visible: true, width: 160 },
  { key: 'employee',            label: 'Employee',        visible: true, width: 180 },
  { key: 'dateSubmitted',       label: 'Date Submitted',  visible: true, width: 130 },
  { key: 'totalAmount',         label: 'Total Amount',    visible: true, width: 130, align: 'right' },
  { key: 'status',              label: 'Status',          visible: true, width: 130 },
]
const STORAGE_KEY = 'reimbursements-cols-v4'
...
const SAMPLE: Reimbursement[] = [
  { id: 'reimb-001', reimbursementNumber: 'REIMB-2026-001', employee: 'Patricia Mendoza', dateSubmitted: '2026-04-08', totalAmount: 7200, status: 'PENDING'    },
  { id: 'reimb-002', reimbursementNumber: 'REIMB-2026-002', employee: 'Jerome Villanueva',dateSubmitted: '2026-04-05', totalAmount: 4360, status: 'APPROVED'   },
  { id: 'reimb-003', reimbursementNumber: 'REIMB-2026-003', employee: 'Aira Cruz',        dateSubmitted: '2026-04-01', totalAmount: 1890, status: 'PAID'        },
  { id: 'reimb-004', reimbursementNumber: 'REIMB-2026-004', employee: 'Marco Reyes',      dateSubmitted: '2026-03-28', totalAmount: 3500, status: 'PROCESSING'  },
]

function compare(a: Reimbursement, b: Reimbursement, key: SortKey, dir: 'asc' | 'desc'): number {
  if (key === 'totalAmount') { const d = a.totalAmount - b.totalAmount; return dir === 'asc' ? d : -d }
...
const STATUSES = ['ALL', 'DRAFT', 'PENDING', 'APPROVED', 'PROCESSING', 'PAID', 'REJECTED']

export default function ReimbursementsPage() {
  const { currency } = useCompanyCurrency()
  const [rows]                          = useState<Reimbursement[]>(SAMPLE)
  ...
  const filtered = useMemo(() => {
    ...
    if (search) { const q = search.toLowerCase(); list = list.filter(r => (r.reimbursementNumber ?? '').toLowerCase().includes(q) || (r.employee ?? '').toLowerCase().includes(q)) }
    if (dateFrom) list = list.filter(r => r.dateSubmitted >= dateFrom)
    if (dateTo)   list = list.filter(r => r.dateSubmitted <= dateTo)
    return list
  ...
  const handleExportCSV = () => { setShowExport(false); csvDownload(`reimbursements-${new Date().toISOString().slice(0,10)}.csv`,['Reimbursement #','Employee','Date Submitted','Total Amount','Status'],sorted.map(r=>[r.reimbursementNumber??'',r.employee??'',r.dateSubmitted,String(r.totalAmount),r.status??'']));showToast('CSV exported') }
  const visibleCols = cols.filter(c => c.visible)

  const renderCell = (row: Reimbursement, key: string) => {
    switch (key) {
      case 'reimbursementNumber': return <span className="font-semibold text-gray-800">{row.reimbursementNumber ?? '—'}</span>
      case 'employee':            return <span className="font-medium text-gray-800">{row.employee ?? '—'}</span>
      case 'dateSubmitted':       return <span className="text-gray-500">{fmtDate(row.dateSubmitted)}</span>
      case 'totalAmount':         return <span className="font-semibold text-emerald-800 tabular-nums">{fmt(row.totalAmount)}</span>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold text-emerald-900">Reimbursements</h1><p className="text-sm text-emerald-600/70 mt-0.5">{`${sorted.length} reimbursements`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => showToast('Coming soon')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Plus size={15} /> New Claim</button>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-emerald-100 p-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-400" /><input type="text" placeholder="Search reimbursements..." value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} className="w-full pl-9 pr-3 py-2 text-sm border border-emerald-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/30" /></div>
      ...
      {selected.size > 0 && (<div className="bg-emerald-600 text-white rounded-xl px-4 py-2.5 flex items-center gap-3"><CheckSquare size={16} /><span className="text-sm font-semibold">{selected.size} selected</span><div className="flex items-center gap-2 ml-auto"><button onClick={() => { csvDownload(`reimb-sel.csv`,['Reimbursement #','Employee','Date Submitted','Total Amount','Status'],sorted.filter(r=>selected.has(r.id)).map(r=>[r.reimbursementNumber??'',r.employee??'',r.dateSubmitted,String(r.totalAmount),r.status??'']));showToast('CSV exported')}} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-xs font-semibold"><Download size={12}/> Export</button><button onClick={()=>setSelected(new Set())} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg"><X size={14}/></button></div></div>)}
      ...
      {actionMenuId&&menuPos&&(()=>{const row=rows.find(r=>r.id===actionMenuId);if(!row)return null;const ml=Math.min(Math.max(4,menuPos.x-208),(typeof window!=='undefined'?window.innerWidth:800)-212);const mt=Math.min(menuPos.y+4,(typeof window!=='undefined'?window.innerHeight:600)-160);return(<div style={{position:'fixed',top:mt,left:ml,zIndex:9999}} className="bg-white border border-gray-200 rounded-xl shadow-xl py-1 w-52"><div className="px-3 py-1.5 border-b border-gray-100"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{row.reimbursementNumber??'Reimbursement'}</p></div><MenuBtn icon={<Eye size={13}/>} label="View Details" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>{row.status==='PENDING'&&<MenuBtn icon={<Check size={13}/>} label="Approve" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>}{row.status==='APPROVED'&&<MenuBtn icon={<Play size={13}/>} label="Process Payment" onClick={()=>{showToast('Coming soon');setActionMenuId(null)}}/>}</div>)})()}
