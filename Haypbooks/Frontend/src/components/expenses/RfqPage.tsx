...
export default function RfqPage() {
  ...
  const table = useEnhancedTable<Rfq>({ data: paged, tableId: 'rfq' })
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      {table.renderBulkToolbar([
        { label: 'Export Selected', onClick: () => table.exportSelectedToCsv(`rfq-selected-${new Date().toISOString().slice(0, 10)}.csv`, ['RFQ #','Subject','Vendors','Date Sent','Closing Date','Status'], (row) => [row.rfqNumber ?? '', row.subject ?? '', String(row.vendorCount ?? 0), row.dateSent ?? '', row.closingDate ?? '', row.status ?? '']) },
      ])}

      <EnhancedTable columns={columns} data={paged} onSort={toggleSort} sortKey={sortKey} sortDir={sortDir} tableId="rfq" hasStickyActions={true} emptyMessage={loading ? 'Loading...' : 'No RFQs found'} rowClassName={(row) => (table.selectedRows.includes(row.id) ? 'bg-blue-50/20' : '')} onColumnsChange={handleColumnsChange}
