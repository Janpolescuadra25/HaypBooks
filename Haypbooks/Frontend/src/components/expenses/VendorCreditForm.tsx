      ...
      <main className="flex-1 min-h-0 overflow-y-auto">
        <div className="mx-auto max-w-7xl px-4 py-2 sm:px-6 lg:px-8">
          <div className="inline-flex rounded-xl bg-white/50 p-1 border border-slate-100">
            ...
            <button type="button" onClick={() => setActiveTab('activity')} disabled={mode === 'new' || !creditId} className={`px-4 py-2 text-sm font-semibold rounded-r-lg ${activeTab === 'activity' ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-slate-50'}`}>Activity</button>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_280px]">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-900">Credit Number</label>
                    <input value={creditNumber ? creditNumber : 'Auto-generated'} readOnly className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-slate-500" />
                  </div>
