...
function deriveTotals(values: FormValues) {
  ...
  // support subtotal marker lines (description === 'Subtotal')
...
export default function NewInvoiceForm() {
  ...
  type InvoiceSettings = {
    ...
    header: { terms: boolean; dueDate: boolean; shipTo: boolean }
    showShipping: boolean
    customFieldNames: string[]
  }
  const defaultSettings: InvoiceSettings = { sales: true, discount: true, deposit: false, customNumbers: true, tags: true, optCols: { date: false, productService: true, sku: false }, header: { terms: true, dueDate: true, shipTo: false }, showShipping: true, customFieldNames: [] }
  ...
  function applyLayoutToSettings(layout: InvoiceLayout) {
    setSettings(prev => {
      const next = {
        ...prev,
        discount: !!layout.showDiscount,
        deposit: !!layout.showDeposit,
        tags: !!layout.showTags,
        // Respect per-view editor visibility for shipping if provided; otherwise fall back
        header: { terms: !!layout.showTerms, dueDate: !!layout.showTerms && !!layout.showDueDate, shipTo: !!(layout.showShippingAddressEditor ?? layout.showShippingAddress) },
        showShipping: !!(layout.showShippingEditor ?? true),
        optCols: { ...prev.optCols, date: !!layout.showServiceDate, productService: true, sku: !!layout.showSKU }
      }
      try { if (JSON.stringify(prev) === JSON.stringify(next)) return prev } catch {}
      return next
    })
  }
  useEffect(() => { let alive = true; (async () => { const latest = await loadLatestLayout(); if (alive && latest) { try { if (currentLayout && JSON.stringify(currentLayout) === JSON.stringify(latest)) return } catch {} applyLayoutToSettings(latest); setCurrentLayout(latest) } })(); return () => { alive = false } }, [currentLayout])
  ...
  const balanceDue = useMemo(()=> Math.max(0, Number((totals.total - Number(deposit||0)).toFixed(2))), [totals.total, deposit])

  useEffect(() => { const prev = document.body.style.overflow; document.body.style.overflow = 'hidden'; return () => { document.body.style.overflow = prev } }, [])

  // Settings persistence & discount sync
  useEffect(() => { try { const raw = localStorage.getItem('invoiceSettings'); if (raw) setSettings({ ...defaultSettings, ...JSON.parse(raw) }) } catch {} }, [])
  useEffect(() => { try { const raw = localStorage.getItem('hb.invoice.autosave'); if (raw) { const saved = JSON.parse(raw); reset({ number: saved.number ?? defaultNumber, customerId: saved.customerId ?? '', customerEmail: saved.customerEmail ?? '', cc: saved.cc ?? '', bcc: saved.bcc ?? '', date: saved.date ?? new Date().toISOString().slice(0, 10), terms: saved.terms ?? 'Net 30', location: saved.location ?? '', amountsMode: saved.amountsMode ?? 'exclusive', lines: Array.isArray(saved.lines) && saved.lines.length ? saved.lines : [{ description:'', qty:1, rate:0, taxable:false }], discountPct: Number(saved.discountPct ?? 0), taxPct: Number(saved.taxPct ?? 0), deposit: Number(saved.deposit ?? 0), applyDiscountAfterTax: !!saved.applyDiscountAfterTax, makeRecurring: saved.makeRecurring ?? { enabled: false, frequency: 'monthly', startDate: new Date().toISOString().slice(0,10) }, customFields: saved.customFields ?? {} }) } } catch {} }, [reset, defaultNumber])
  useEffect(() => { try { const raw = localStorage.getItem('hb.invoice.layout'); if (raw) { const latest = JSON.parse(raw); setValue('applyDiscountAfterTax', !!latest.applyDiscountAfterTax, { shouldDirty: true }) } } catch {} }, [setValue])
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      try { localStorage.removeItem('hb.invoice.autosave') } catch {}
      router.push(toHref('/invoices'))
    } catch (e: any) { setError(e.message || 'Failed to create invoice') }
  }

  const autosave = watch(); const [autosaveStatus, setAutosaveStatus] = useState<'idle'|'saving'|'saved'>('idle'); const [lastSavedAt, setLastSavedAt] = useState<number | null>(null)
  useEffect(() => { setAutosaveStatus('saving'); const id = setTimeout(() => { try { localStorage.setItem('hb.invoice.autosave', JSON.stringify(autosave)); setAutosaveStatus('saved'); setLastSavedAt(Date.now()) } catch { setAutosaveStatus('idle') } }, 700); return () => clearTimeout(id) }, [autosave])
  useEffect(() => { function beforeUnload(e: BeforeUnloadEvent) { if (isDirty) { e.preventDefault(); e.returnValue = '' } } window.addEventListener('beforeunload', beforeUnload); return () => window.removeEventListener('beforeunload', beforeUnload) }, [isDirty])
    ...
    // animation mount/visibility controls for discard confirmation modal
    ...
    function confirmDiscard() { setShowDiscardConfirm(false); window.setTimeout(() => router.back(), DISCARD_ANIM_MS) }
  ...
  // Opening animation for the invoices variant main dialog
  const [isDialogVisible, setIsDialogVisible] = useState(false)
  useEffect(() => { const t = window.setTimeout(() => setIsDialogVisible(true), 10); return () => window.clearTimeout(t) }, [])
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div
        ...
        className={`absolute flex flex-col !bg-white rounded-2xl border border-slate-200/60 p-4 md:p-6 transition-all duration-300 shadow-[0_12px_24px_rgba(15,23,42,0.22),_0_36px_72px_rgba(15,23,42,0.28),_0_72px_144px_rgba(15,23,42,0.24)] animate-[hb-border-sheen_7.5s_ease-in-out_infinite] inset-0 m-2 md:m-4 w-[calc(100%-1rem)] md:w-[calc(100%-2rem)] h-[calc(100%-1rem)] md:h-[calc(100%-2rem)] transform ${isDialogVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-3 scale-95'}`}>
        <div className="flex items-center justify-between mb-3 shrink-0 bg-white border-b border-slate-200 pb-2">
          ...
          <div className="flex items-center gap-2">
            <div className="text-xs text-slate-500 pr-2 select-none" aria-live="polite">{autosaveStatus === 'saving' && 'Saving…'}{autosaveStatus === 'saved' && lastSavedAt && `Saved ${new Date(lastSavedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}</div>
        ...
        {isDiscardMounted && typeof document !== 'undefined' && createPortal(
          <div role="dialog" aria-modal="true" aria-labelledby="discard-title" className="fixed left-1/2 top-[40%] z-[80] w-[min(520px,94%)] transform -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <div className={`bg-white rounded-xl shadow-[0_30px_80px_rgba(2,6,23,0.55),_0_16px_40px_rgba(2,6,23,0.36)] z-[90] w-full p-5 border border-slate-200 transition-all duration-240 ease-out transform ${isDiscardVisible ? 'opacity-100 translate-y-0 scale-100 pointer-events-auto' : 'opacity-0 translate-y-4 scale-95'}`}>
      ...
      {/* discard-overlay mounted at top-level so fixed positioning covers the viewport */}
      {isDiscardMounted && typeof document !== 'undefined' && createPortal(
        <div data-testid="discard-overlay" className={`fixed inset-0 z-[70] bg-black/40 transition-opacity duration-200 ${isDiscardVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={cancelDiscard} />,
        document.body
      )}
        <div className={`flex-1 min-h-0 ${isDiscardMounted ? 'overflow-hidden' : 'overflow-y-auto'} pr-1 relative`}>
          ...
          {activeTab === 'invoice' ? (
            <form id="invoice-form" noValidate onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <input id="inv-number" className="w-full rounded-xl bg-white border border-slate-200 px-3 py-2 outline-none focus:ring-2 focus:ring-sky-400/50" readOnly={!settings.customNumbers} aria-invalid={!!errors.number} {...register('number')} />
                  {errors.number && <p className="text-red-600 text-sm mt-1" id="inv-number-error">{errors.number.message}</p>}
                ...
                <div>
                  ...
                  <input id="inv-date" type="date" className="w-full rounded-xl bg-white text-slate-900 border border-slate-200 px-3 py-2 outline-none focus:ring-2 focus:ring-sky-400/50" aria-invalid={!!errors.date} {...register('date')} />
                  {errors.date && <p className="text-red-600 text-sm mt-1" id="inv-date-error">{errors.date.message}</p>}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    <CustomerDropdown id="inv-customer" value={watch('customerId') || ''} onSelect={(id: string) => setValue('customerId', id, { shouldDirty: true })} customers={customers.map(c => ({ id: c.id, name: c.name }))} ariaDescribedBy={undefined} placeholder="Select customer…" />
                  </div>
                  {errors.customerId && <p className="text-red-600 text-sm mt-1" id="inv-customer-error">{errors.customerId.message}</p>}
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-slate-600">Cc</label><input type="text" className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('cc')} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-600">Bcc</label><input type="text" className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('bcc')} />
                    </div>
                  </div>
                  <div className="mt-3">
                ...
                <div>
                  ...
                  <div className="mt-4"><label className="block text-sm font-medium text-slate-700">Billing address</label><textarea id="inv-billing-address" aria-label="Billing address" title="Billing address" readOnly rows={4} className="w-full rounded-xl bg-slate-50 border border-slate-200 px-3 py-2" value={(customers.find(c=>c.id===customerId)?.billingAddress)||''} /></div>
                  {settings.header.shipTo && (<div className="mt-4"><label className="block text-sm font-medium text-slate-700">Shipping address</label><textarea id="inv-shipping-address" aria-label="Shipping address" title="Shipping address" readOnly rows={4} className="w-full rounded-xl bg-slate-50 border border-slate-200 px-3 py-2" value={(customers.find(c=>c.id===customerId)?.billingAddress)||''} /></div>)}
                </div>
              </div>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
                    ...
                    <table className="min-w-full text-sm">
                      ...
                      <thead className="text-slate-600">
                        <tr>
                          <th className="w-8 border-r border-slate-200" aria-hidden><div className="relative h-full" /></th>
                          {settings.optCols.productService && <ResizableColumn colKey="productService" width={colWidths.productService} onChange={updateColWidth} className="px-2 py-2 text-left border-r border-slate-200">Product/Service</ResizableColumn>}
                          {settings.optCols.sku && <ResizableColumn colKey="sku" width={colWidths.sku} onChange={updateColWidth} className="px-2 py-2 text-left border-r border-slate-200">SKU</ResizableColumn>}
                          {settings.optCols.date && <ResizableColumn colKey="date" width={colWidths.date} onChange={updateColWidth} className="px-2 py-2 text-left border-r border-slate-200">Date</ResizableColumn>}
                      ...
                      <tbody className="text-slate-800">{
                        (() => {
                          ...
                          return (totals.withAmounts || []).map((entry: any, displayIdx: number) => {
                            ...
                            // Each processed entry lines up with the backing fields by index
                            ...
                            // mapping is index-aligned; no extra pointer advance needed
                              return (
                              <tr key={key} className={`border-t border-slate-200 ${draggingIdx===displayIdx ? 'opacity-60' : ''} ${dragOverIdx===displayIdx ? 'ring-1 ring-sky-200' : ''}`} data-index={displayIdx} onDragOver={(e) => handleDragOver(e, displayIdx)} onDrop={(e) => handleDrop(e, displayIdx)} onDragEnd={handleDragEnd} aria-grabbed={draggingIdx === displayIdx ? 'true' : 'false'}>
                                <td className="px-3 py-2 align-middle w-8">
                                  ...
                                  >
                                    ...
                                    {/* Drag handle - click & hold to drag row */}
                                    <button
                                      aria-label={`Drag row ${displayIdx}`}
                                      title="Drag to reorder"
                                      draggable
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700">Internal memo</label><textarea className="w-full rounded-xl bg-white border border-slate-200 px-3 py-2" rows={3} {...register('memo')} /></div>
                <div><label className="block text-sm font-medium text-slate-700">Message on statement</label><textarea className="w-full rounded-xl bg-white border border-slate-200 px-3 py-2" rows={3} {...register('messageOnStatement')} /></div>
                {settings.customFieldNames.length > 0 && (<div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">{settings.customFieldNames.map(name => (<div key={name}><label className="block text-sm font-medium text-slate-700">{name}</label><input className="w-full rounded-xl bg-white border border-slate-200 px-3 py-2" {...register(`customFields.${name}` as const)} /></div>))}</div>)}
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-2 text-sm">
                ...
                {!applyDiscountAfterTax && (<>{settings.discount && (<div className="flex justify-between"><label className="flex items-center gap-2">Discount %<input type="number" min={0} max={100} step={0.01} className="w-20 rounded border border-slate-300 px-2 py-1 text-right" {...register('discountPct')} /></label><span>-{totals.discountAmt.toFixed(2)}</span></div>)}<div className="flex justify-between"><span>Taxable Subtotal</span><span>{totals.taxableSubtotal.toFixed(2)}</span></div><div className="flex justify-between"><label className="flex items-center gap-2">Sales Tax %<input type="number" min={0} max={100} step={0.01} className="w-20 rounded border border-slate-300 px-2 py-1 text-right" {...register('taxPct')} /></label><span>+{totals.taxAmt.toFixed(2)}</span></div></>)}
                {applyDiscountAfterTax && (<><div className="flex justify-between"><span>Taxable Subtotal</span><span>{totals.taxableSubtotal.toFixed(2)}</span></div><div className="flex justify-between"><label className="flex items-center gap-2">Sales Tax %<input type="number" min={0} max={100} step={0.01} className="w-20 rounded border border-slate-300 px-2 py-1 text-right" {...register('taxPct')} /></label><span>+{totals.taxAmt.toFixed(2)}</span></div>{settings.discount && (<div className="flex justify-between"><label className="flex items-center gap-2">Discount %<input type="number" min={0} max={100} step={0.01} className="w-20 rounded border border-slate-300 px-2 py-1 text-right" {...register('discountPct')} /></label><span>-{totals.discountAmt.toFixed(2)}</span></div>)}</>)}
                ...
                {settings.deposit && (<div className="flex justify-between"><label className="flex items-center gap-2">Payment received<input type="number" min={0} step={0.01} className="w-28 rounded border border-slate-300 px-2 py-1 text-right" {...register('deposit')} /></label><span>-{Number(deposit||0).toFixed(2)}</span></div>)}
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
                ...
                {recurring?.enabled && (<div className="grid grid-cols-1 sm:grid-cols-5 gap-4 text-sm"><div><label className="block text-xs font-medium text-slate-600">Frequency</label><select className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('makeRecurring.frequency')}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="yearly">Yearly</option></select></div><div><label className="block text-xs font-medium text-slate-600">Start Date</label><input type="date" className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('makeRecurring.startDate')} /></div><div><label className="block text-xs font-medium text-slate-600">End Date</label><input type="date" className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('makeRecurring.endDate')} /></div><div><label className="block text-xs font-medium text-slate-600">Occurrences</label><input type="number" min={1} className="w-full rounded-md border border-slate-200 px-2 py-1" {...register('makeRecurring.occurrences')} /></div><div className="flex items-end"><p className="text-xs text-slate-500">Template created after save.</p></div></div>)}
            ...
            <div className="space-y-3">
              <InvoiceLayoutPanel initialTab={desiredLayoutTab} onLayoutChange={(l)=> { try { if (currentLayout && JSON.stringify(currentLayout) === JSON.stringify(l)) return } catch {} setCurrentLayout(l); applyLayoutToSettings(l) }} previewData={{ invoice: { number: watch('number'), terms, invoiceDate: date, dueDate: dueDatePreview, billTo: (customers.find(c=>c.id===customerId)?.name || 'Client Co.') + (customers.find(c=>c.id===customerId)?.billingAddress ? `\n${customers.find(c=>c.id===customerId)?.billingAddress}` : ''), shipTo: settings.header.shipTo ? (customers.find(c=>c.id===customerId)?.billingAddress || '') : undefined, items: (linesWatch||[]).map(l => ({ name: l.description || 'Item', qty: Number(l.qty||0), rate: Number(l.rate||0), sku: l.sku || undefined, date: l.date || undefined })), discountAmount: totals.discountAmt, paymentsReceived: Number(deposit||0), shipping: 0 } }} onBack={()=> { try { const raw = localStorage.getItem('hb.invoice.layout'); if (raw) { const parsed = JSON.parse(raw); try { if (currentLayout && JSON.stringify(currentLayout) === JSON.stringify(parsed)) { setActiveTab('invoice'); return } } catch {} applyLayoutToSettings(parsed); setCurrentLayout(parsed); if (watch('applyDiscountAfterTax') !== !!parsed.applyDiscountAfterTax) setValue('applyDiscountAfterTax', !!parsed.applyDiscountAfterTax, { shouldDirty: true }) } } catch {}; setActiveTab('invoice') }} />
            </div>
          )}
        </div>
        <div className="pt-3 mt-2 border-t bg-white flex items-center justify-between gap-2 shrink-0">
          ...
          {activeTab === 'invoice' && (<div className="flex gap-2"><button type="button" className="btn-secondary" disabled={isSubmitting}>Print / Preview</button><button form="invoice-form" type="submit" className="btn-primary" disabled={isSubmitting || isBlocked}>{isSubmitting ? 'Saving…' : 'Save & Send'}</button></div>)}
        </div>
      </div>
    </div>
  )
