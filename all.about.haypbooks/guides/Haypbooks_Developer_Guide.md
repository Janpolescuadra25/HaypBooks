# Haypbooks Online Developer Guide
...
This developer guide provides comprehensive documentation for integrating with Haypbooks Online through APIs, building custom applications, and implementing enterprise-level features. Designed for developers who need to extend, integrate, or customize Haypbooks functionality.
...
This section documents the shared rules every CSV export must follow so behavior is consistent across reports and lists.

- JSON-first source of truth
  - Every CSV export route delegates to its JSON sibling route. The CSV layer must not reconstruct data independently. This prevents drift and centralizes business logic.
  - Example: `reports/<name>/export/route.ts` calls `reports/<name>/route.ts` and formats the returned caption/header/rows/totals.

- Layout and metadata
  - When explicitly requested, include an opt-in CSV metadata line as the first row: `CSV-Version,1`.
  - Follow this sequence: caption row, a blank spacer row, header row, data rows, and an optional totals row when applicable.
  - Use shared helpers for consistency: `buildCsvCaption` (or `buildCsvRangeOrDate`) for the caption; consistent header labels per report; and stable totals labels (e.g., `Totals`).

- Filenames and tokens
  - Always build filenames with `buildCsvFilename(slug, opts)`. The helper applies date range or as-of semantics and sanitizes tokens.
  - Default policy: statements and most list/summary reports use tokens-after-asof. Some compare-mode core statements may place tokens-before-asof as documented in the tokens guide.
  - See: CSV filename tokens guide for detailed rules and examples.

- Print UX
  - Maintain a single browser Print button. Do not introduce print-only routes. Legacy print routes must redirect to canonical locations.

- Period locks and accounting safeguards
  - Exports must respect period-lock rules and allocation safeguards implemented in JSON handlers. The CSV layer should propagate any resulting API errors using the established error shapes.

- Tests and guardrails
  - Add or update focused tests when introducing or changing an export. Validate: CSV-Version behavior, caption/header layout, totals, and filename shape (including tokens and as-of semantics).
  - The brand-terms guard applies to frontend source; keep in-app copy brand-neutral. Documentation may reference external systems where appropriate.

These conventions are enforced across existing exports (journals, ledgers, statements, tax, reconciliation, vendor/customer lists). New exports should follow the same pattern.

## Receipts: matching suggestions and preview

The Receipts workflow provides an advisory endpoint to suggest potential matches against open documents (accounts receivable and accounts payable) without mutating state.

- Endpoint: `GET /api/receipts/{id}/suggestions`
  - RBAC: requires `reports:read` (read-only)
  - Query params: `limit` (1–25, default 5), `includeBills` (true/false; defaults to true for positive amounts)
  - Response shape:
    - `suggestions[]` with fields:
      - `kind`: `invoice` or `bill`
      - `id`, `number`, `name` (customer or vendor name)
      - `remaining`: current open balance
      - `amountDifference`: absolute difference to receipt amount (currency units)
      - `dateDifferenceDays`: absolute day difference to receipt date (if available)
      - `vendorSimilarity`: numeric 0–1 similarity between receipt vendor text and candidate name
      - `score`: composite ranking score used for ordering (higher is better)

- Ranking model (brand‑neutral parity):
  - Composite score favors:
    - Vendor name similarity (50%) using a lightweight Levenshtein‑based metric with a small prefix boost
    - Amount proximity (30%), normalized by the receipt absolute amount
    - Date proximity (15%), linearly decaying over a ~30‑day window
    - Small tie‑break bias toward invoices on inflow paths
  - Final sort: score desc, then smaller `amountDifference`, then smaller `dateDifferenceDays`, then `kind` (invoice first), then `id`.
  - Amount tolerance for candidates is ±1 currency unit to keep the list concise and deterministic in tests.

- Linked preview: `GET /api/receipts/{id}/linked`
  - Returns a concise, type‑aware preview for a matched document (invoice or bill): number, dates, status, open amount, and line count. This is used by the UI “View match” modal for quick verification before posting.

Testing notes:
- Unit tests validate presence of `vendorSimilarity` and `score` and that vendor‑closer candidates outrank others when amounts/dates tie.
- Deterministic OCR and seed data ensure stable tests. Avoid randomness in parsers and ranking.

### CSV-Version prelude (Added 2025-10-15)

To enable forward-compatible parsing, CSV exports can optionally include a version prelude as the very first row:

- Exact text: CSV-Version,1
- Emission: Only when the request opts in via a csv-version flag (aliases supported). Use parseCsvVersionFlag(req) from frontend/src/lib/csv.ts.
- Placement: Always the first line, before the caption. Then include a blank spacer line before headers.
- Scope: Applies to all CSV exports, including history and operational reports. Tests should assert the first line when opted-in.

Implementation checklist:
- Call parseCsvVersionFlag(req) and, if true, output CSV-Version,1 as the first line.
- Build caption and filename via shared helpers to avoid drift.
- Keep JSON as the source of truth; CSV is a formatting layer over the same data.

See also: frontend/docs/CSV-Filenames.md for filename token rules and examples.


## Journal reversal and period locks

To support common accounting workflows and ensure parity with expected behaviors, the mock backend provides a journal reversal endpoint and simple period-lock utilities.

- Endpoint: `POST /api/journal/{id}/reverse`
  - RBAC: requires `journal:write` permission.
  - Body (optional): `{ "date": "YYYY-MM-DD", "memo": "optional memo" }`
  - Behavior:
    - If `date` is provided and falls on/before the closed-through date, the API returns 400 with guidance to pick a later date.
    - If `date` is omitted, the system selects the next calendar day after the original journal date. If that day is closed, it automatically normalizes to the next open day after the close.
    - The reversing journal swaps debits and credits for each line, preserves account mapping, sets `reversing` flags, and links back to the original (`reversesEntryId`).
  - Response: `{ "reversingEntryId": "je_xxxxxx" }` on success, or `{ "error": string }` with appropriate HTTP status on failure (400/403/404).

- Period locks
  - Utilities live under `src/lib/periods.ts` and expose:
    - `getClosedThrough()` — returns the current closed-through date (inclusive) or `null`.
    - `closeThrough(endISO)` — sets or updates the closed-through date and returns a period record.
    - `isDateAllowed(dateISO)` — boolean helper for UI validation.
  - API: `GET /api/periods` to read current lock state, `POST /api/periods` with `{ closeThrough: "YYYY-MM-DD" }` to update.

Notes:
  - Reversal defaults respect the “next open day” rule and align with lock enforcement across posting helpers.
  - Audit logging for reversals can be added in the future; for now, linked ids and flags enable traceability in mock data.
...
#### API Base URL
```
https://Haypbooks.api.intuit.com/v3/company/{companyId}
```
...
The Haypbooks data model is built around interconnected business entities that reflect real-world business relationships.
...
Haypbooks Online uses OAuth 2.0 for secure API authentication, providing delegated access without sharing user credentials.

#### Authorization Flow
1. **Client Registration**: Register application in Haypbooks Developer Portal
2. **Authorization Request**: Redirect user to Haypbooks authorization URL
3. **User Consent**: User grants permission to access their Haypbooks data
4. **Authorization Code**: Receive authorization code from Haypbooks
...
#### Available Scopes
- **com.intuit.Haypbooks.accounting**: Full accounting access
- **com.intuit.Haypbooks.payroll**: Payroll data access
- **com.intuit.Haypbooks.payment**: Payment processing access
- **com.intuit.Haypbooks.payroll.employee**: Employee data access
- **com.intuit.Haypbooks.payroll.company**: Company payroll settings

#### Scope Examples
```javascript
// Full accounting access
scope: 'com.intuit.Haypbooks.accounting'

// Read-only access
scope: 'com.intuit.Haypbooks.accounting.read'

// Payroll access
scope: 'com.intuit.Haypbooks.payroll com.intuit.Haypbooks.payroll.employee'
```

### Multi-Tenant Support

#### Company Context
- **Company ID**: Unique identifier for each Haypbooks company
...
const apiUrl = `https://Haypbooks.api.intuit.com/v3/company/${companyId}/customer`;
```
...
- Consolidate a source account into a target account by rewriting all references to the target, recalculating balances, and either inactivating or deleting the source when safe.

RBAC

- Requires `journal:write` permission; otherwise returns 403.

Request Body

```json
{
  "sourceId": "acc_src_123",
  "targetId": "acc_tgt_456",
  "strategy": "inactivate" // optional: "inactivate" (default) or "delete"
}
```

Validation Rules

- `sourceId` and `targetId` are required and must differ.
- Source and target accounts must be the same account type.
- Protected system accounts cannot be merged as a source.
- Target account must be active.

Behavior

- Rewrites references in: transactions.accountId, journalEntries.lines[].accountId, items.defaultAccountId, reconcileSessions.accountId.
- Recalculates account balances after rewrites.
- Strategy:
  - `inactivate` (default): marks source inactive and appends "(merged)" to name.
  - `delete`: only allowed when no references remain to the source after rewrites.
- Appends an audit event with counts of updated entities and metadata.
...
- 400 Bad Request: missing fields, same ids, type mismatch, protected source, inactive target, delete while still referenced.
- 403 Forbidden: user lacks `journal:write` permission.
- 404 Not Found: source or target not found.

Examples

Request (default inactivate):

```http
POST /api/accounts/merge HTTP/1.1
Content-Type: application/json

{"sourceId":"acc_src_123","targetId":"acc_tgt_456"}
```

Request (delete when safe):

```http
POST /api/accounts/merge HTTP/1.1
Content-Type: application/json

{"sourceId":"acc_src_123","targetId":"acc_tgt_456","strategy":"delete"}
```

Notes

- Consumers should refresh any cached account lists and ledger summaries after a successful merge.
- When using `delete`, verify no external systems still reference the source id.
...
serviceContext.IppConfiguration.BaseUrl.Qbo = "https://Haypbooks.api.intuit.com/";
...
context.getIppConfiguration().getBaseUrl().setQbo("https://Haypbooks.api.intuit.com/");
...
#### Node.js SDK
```javascript
const Haypbooks = require('node-Haypbooks');

// Initialize Haypbooks instance
const qbo = new Haypbooks(
  consumerKey,
  consumerSecret,
  accessToken,
  refreshToken,
...
#### Python Haypbooks Client
```python
from Haypbooks import Haypbooks
from Haypbooks.objects import Customer
...
#### PHP Haypbooks SDK
```php
require_once 'Haypbooks.php';

// Initialize Haypbooks instance
$Haypbooks = new Haypbooks(
    $consumerKey,
    $consumerSecret,
    $accessToken,
    $accessTokenSecret,
...
// Create customer
$customer = new Haypbooks_Object_Customer();
$customer->setName('New Customer');
$customer->setCompanyName('New Customer Inc.');

$result = $Haypbooks->add($customer);
```
...
#### Sandbox Setup
1. **Create Sandbox Company**: Use Haypbooks Developer Portal to create sandbox
...
#### Sandbox URLs
```
Production: https://Haypbooks.api.intuit.com/v3/company/{companyId}
Sandbox: https://sandbox-Haypbooks.api.intuit.com/v3/company/{companyId}
```
...
async function createTestCustomers(count = 10) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const response = await HaypbooksAPI.createCustomer(customer);
...
async function createTestInvoices(customerIds, count = 5) {
  ...
  for (const customerId of customerIds) {
    for (let i = 1; i <= count; i++) {
      ...
      const response = await HaypbooksAPI.createInvoice(invoice);
...
const HaypbooksAPI = require('../lib/Haypbooks-api');

describe('Haypbooks API Client', () => {
  let apiClient;
  let sandbox;
  
  beforeEach(() => {
    sandbox = sinon.createSandbox();
    apiClient = new HaypbooksAPI({
...
describe('Haypbooks Integration Tests', () => {
  let driver;
...
const apiExplorer = {
  baseUrl: 'https://sandbox-Haypbooks.api.intuit.com/v3/company',
  endpoints: {
    customers: '/customer',
    invoices: '/invoice',
    payments: '/payment',
...
const webhookConfig = {
  ...
  endpoints: {
    customer: '/Haypbooks/customer',
    invoice: '/Haypbooks/invoice',
    payment: '/Haypbooks/payment',
    bill: '/Haypbooks/bill',
    item: '/Haypbooks/item'
  },
  ...
  registerWebhook: async function(webhook) {
    const response = await fetch('https://Haypbooks.api.intuit.com/v3/company/' + companyId + '/webhooks', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
...
class WebhookHandler {
  ...
  async handleInvoiceWebhook(req, res) {
    ...
    try {
      const invoice = await this.getInvoiceFromHaypbooks(entityId);
  ...
  async handlePaymentWebhook(req, res) {
    ...
    try {
      const payment = await this.getPaymentFromHaypbooks(entityId);
  ...
  async getInvoiceFromHaypbooks(invoiceId) {
    const response = await fetch(`https://Haypbooks.api.intuit.com/v3/company/${companyId}/invoice/${invoiceId}`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
      }
  ...
  async syncNewCustomer(customerId) {
    const customer = await this.getCustomerFromHaypbooks(customerId);
  ...
  async getCustomerFromHaypbooks(customerId) {
    const response = await fetch(`https://Haypbooks.api.intuit.com/v3/company/${companyId}/customer/${customerId}`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
      }
  ...
  async getPaymentFromHaypbooks(paymentId) {
    const response = await fetch(`https://Haypbooks.api.intuit.com/v3/company/${companyId}/payment/${paymentId}`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json'
      }
...
class BatchProcessor {
  constructor(companyId, accessToken) {
    ...
    this.baseUrl = 'https://Haypbooks.api.intuit.com/v3/company';
    this.maxBatchSize = 25; // Haypbooks limit
  }
...
class LargeDatasetProcessor {
  constructor(companyId, accessToken) {
    ...
    this.accessToken = accessToken;
    this.baseUrl = 'https://Haypbooks.api.intuit.com/v3/company';
    this.maxResults = 1000; // Haypbooks max per request
  }
...
// Express route
app.post('/webhooks/Haypbooks', (req, res) => {
...
class HaypbooksErrorHandler {
  ...
  async handleError(error, context = {}) {
    ...
    console.error('Haypbooks API Error:', {
      ...classifiedError,
      context,
      stack: error.stack
    });
...
// Usage
const errorHandler = new HaypbooksErrorHandler();

try {
  // API call that might fail
  await HaypbooksAPI.createCustomer(customerData);
} catch (error) {
...
### Domain error shapes and period-lock enforcement

Closed period and allocation business rules surface as validation errors with HTTP 400 and a descriptive message. Missing entities continue to use 404. This keeps UIs and tests deterministic and prevents masking domain errors as not-found.

- Examples (400 Bad Request):
  - "Cannot post to closed period ending 2025-08-31"
  - "Allocation exceeds remaining open balance on invoice INV-1001"
- Example (404 Not Found):
  - "Invoice not found" when the invoice id does not exist or is not visible

Posting rules
- All journal postings normalize the posting date to `YYYY-MM-DD` and assert the target period is open. Reversing entries default to the next open date when the requested date is closed.
- Customer payments, deposits, vendor credits, and bill payments assert an open period before mutating balances.
- API routes should preserve the following mapping:
  - 400 for closed-period and allocation violations
  - 403 for RBAC violations
  - 404 for entity-not-found

Client guidance
- Surface 400-validation messages directly in the UI error banner/toast.
- Avoid generic 404 for domain/business rule errors.
- Tests may assert specific error messages for closed-period attempts and allocation limits.
...
try {
  ...
  const recoveryResult = await recoveryManager.recoverFromError(error, {
    ...
    accessToken,
    refreshToken,
    retryFunction: () => HaypbooksAPI.createCustomer(customerData)
  });
  ...
  async addCompany(companyConfig) {
    ...
    const company = {
      ...
      baseUrl: environment === 'sandbox' 
        ? 'https://sandbox-Haypbooks.api.intuit.com/v3/company'
        : 'https://Haypbooks.api.intuit.com/v3/company',
      lastUsed: new Date(),
...
class ShopifyIntegration {
  constructor(config) {
    ...
    this.Haypbooks = config.HaypbooksClient;
  }
  
  // Sync products from Shopify to Haypbooks
  async syncProducts() {
    try {
  ...
  // Sync orders from Shopify to Haypbooks
  ...
  // Sync customers from Shopify to Haypbooks
  async syncCustomers() {
    try {
  ...
  async createQBItem(product) {
    ...
    const response = await this.Haypbooks.makeAPIRequest('item', {
      method: 'POST',
      body: JSON.stringify(itemData)
    });
    
    if (!response.ok) {
      throw new Error(`Haypbooks item creation failed: ${response.statusText}`);
    }
  ...
  async updateQBItem(itemId, product) {
    ...
    const response = await this.Haypbooks.makeAPIRequest(`item/${itemId}`, {
      method: 'POST',
      body: JSON.stringify(updateData)
    });
    
    if (!response.ok) {
      throw new Error(`Haypbooks item update failed: ${response.statusText}`);
    }
  ...
  async findQBItemBySKU(sku) {
    ...
    const response = await this.Haypbooks.makeAPIRequest(`item?where=SKU='${sku}'`);
  ...
  async createQBInvoiceFromOrder(order) {
    ...
    const response = await this.Haypbooks.makeAPIRequest('invoice', {
      method: 'POST',
      body: JSON.stringify(invoiceData)
    });
    
    if (!response.ok) {
      throw new Error(`Haypbooks invoice creation failed: ${response.statusText}`);
    }
  ...
  async createQBCustomer(shopifyCustomer) {
    ...
    const response = await this.Haypbooks.makeAPIRequest('customer', {
      method: 'POST',
      body: JSON.stringify(customerData)
    });
    
    if (!response.ok) {
      throw new Error(`Haypbooks customer creation failed: ${response.statusText}`);
    }
  ...
  async updateQBCustomer(customerId, shopifyCustomer) {
    ...
    const response = await this.Haypbooks.makeAPIRequest(`customer/${customerId}`, {
      method: 'POST',
      body: JSON.stringify(updateData)
    });
    
    if (!response.ok) {
      throw new Error(`Haypbooks customer update failed: ${response.statusText}`);
    }
  ...
  async findQBCustomerByEmail(email) {
    ...
    const response = await this.Haypbooks.makeAPIRequest(`customer?where=PrimaryEmailAddr.Address='${email}'`);
...
const shopifyIntegration = new ShopifyIntegration({
  ...
  HaypbooksClient: HaypbooksClient
});
...
class SAPIntegration {
  ...
  async startSyncProcess() {
    console.log('Starting SAP to Haypbooks sync process...');
  ...
  async performFullSync() {
    console.log('Performing full SAP to Haypbooks sync...');
  ...
  async syncCustomer(sapCustomer) {
    // Check if customer exists in Haypbooks
    ...
    if (existingCustomer) {
      ...
      await this.Haypbooks.makeAPIRequest(`customer/${existingCustomer.Id}`, {
        method: 'POST',
        body: JSON.stringify(customerData)
      });
    } else {
      // Create new customer
      await this.Haypbooks.makeAPIRequest('customer', {
        method: 'POST',
        body: JSON.stringify(customerData)
      });
    }
  ...
  async findQBCustomerBySAPId(sapId) {
    const response = await this.Haypbooks.makeAPIRequest(`customer?where=CustomField[1].StringValue='${sapId}'`);
  ...
  async syncVendor(sapVendor) {
    ...
    if (existingVendor) {
      ...
      await this.Haypbooks.makeAPIRequest(`vendor/${existingVendor.Id}`, {
        method: 'POST',
        body: JSON.stringify(vendorData)
      });
    } else {
      await this.Haypbooks.makeAPIRequest('vendor', {
        method: 'POST',
        body: JSON.stringify(vendorData)
      });
    }
  ...
  async findQBVendorBySAPId(sapId) {
    const response = await this.Haypbooks.makeAPIRequest(`vendor?where=CustomField[2].StringValue='${sapId}'`);
  ...
  async syncItem(sapItem) {
    ...
    if (existingItem) {
      ...
      await this.Haypbooks.makeAPIRequest(`item/${existingItem.Id}`, {
        method: 'POST',
        body: JSON.stringify(itemData)
      });
    } else {
      await this.Haypbooks.makeAPIRequest('item', {
        method: 'POST',
        body: JSON.stringify(itemData)
      });
    }
  ...
  async findQBItemBySAPId(sapId) {
    const response = await this.Haypbooks.makeAPIRequest(`item?where=CustomField[3].StringValue='${sapId}'`);
  ...
  async syncOrder(sapOrder) {
    ...
    await this.Haypbooks.makeAPIRequest('estimate', {
      method: 'POST',
  ...
  async syncInvoice(sapInvoice) {
    ...
    await this.Haypbooks.makeAPIRequest('invoice', {
      method: 'POST',
      body: JSON.stringify(invoiceData)
    });
  }
...
class SalesforceIntegration {
  ...
  // Sync Salesforce Accounts to Haypbooks Customers
  async syncAccounts() {
    console.log('Syncing Salesforce Accounts to Haypbooks Customers...');
  ...
  // Sync Salesforce Contacts to Haypbooks Customers
  async syncContacts() {
    console.log('Syncing Salesforce Contacts to Haypbooks Customers...');
  ...
  // Sync Salesforce Opportunities to Haypbooks Estimates
  async syncOpportunities() {
    console.log('Syncing Salesforce Opportunities to Haypbooks Estimates...');
  ...
  // Sync Salesforce Orders to Haypbooks Invoices
  async syncOrders() {
    console.log('Syncing Salesforce Orders to Haypbooks Invoices...');
  ...
  async findQBCustomerBySFId(sfId) {
    const response = await this.Haypbooks.makeAPIRequest(`customer?where=CustomField[1].StringValue='${sfId}' OR CustomField[2].StringValue='${sfId}'`);
  ...
  // Bidirectional sync - Haypbooks to Salesforce
  async syncQBCustomersToSalesforce() {
    console.log('Syncing Haypbooks Customers to Salesforce...');
  ...
  async syncQBCustomerToSalesforce(qbCustomer) {
    ...
    if (existingAccount) {
      ...
      // Create new account
      accountData.Haypbooks_ID__c = qbCustomer.Id; // Custom field
  ...
  async findSalesforceAccountByQBId(qbId) {
    const result = await this.sfConnection.query(
      `SELECT Id FROM Account WHERE Haypbooks_ID__c = '${qbId}'`
    );
  ...
  async getQBCustomersModifiedSince(since) {
    const response = await this.Haypbooks.makeAPIRequest(`customer?where=MetaData.LastUpdatedTime > '${since}'`);
...
class StripeIntegration {
  ...
  // Sync Stripe customers to Haypbooks
  async syncCustomers() {
    console.log('Syncing Stripe customers to Haypbooks...');
  ...
  // Sync Stripe charges to Haypbooks payments
  async syncCharges(startDate = null) {
    console.log('Syncing Stripe charges to Haypbooks payments...');
  ...
  // Sync Stripe subscriptions to Haypbooks recurring invoices
  async syncSubscriptions() {
    console.log('Syncing Stripe subscriptions to Haypbooks...');
  ...
  async syncStripeChargeToQBPayment(stripeCharge) {
    ...
    await this.Haypbooks.makeAPIRequest('payment', {
      method: 'POST',
  ...
  async syncStripeSubscriptionToQB(stripeSubscription) {
    ...
    await this.Haypbooks.makeAPIRequest('recurringtransaction', {
      method: 'POST',
  ...
  async findQBCustomerByStripeId(stripeId) {
    const response = await this.Haypbooks.makeAPIRequest(`customer?where=CustomField[1].StringValue='${stripeId}'`);
  ...
  async findQBInvoiceByStripeId(stripeInvoiceId) {
    const response = await this.Haypbooks.makeAPIRequest(`invoice?where=CustomField[4].StringValue='${stripeInvoiceId}'`);
  ...
  async handleRefund(stripeRefund) {
    ...
    if (qbPayment) {
      ...
      await this.Haypbooks.makeAPIRequest('creditmemo', {
        method: 'POST',
  ...
  async findQBPaymentByStripeId(stripeChargeId) {
    const response = await this.Haypbooks.makeAPIRequest(`payment?where=PaymentRefNum='${stripeChargeId}'`);
...
await fetch('https://Haypbooks.api.intuit.com/v3/company/123/customer', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${accessToken}`,
    'Content-Type': 'application/json',
...
class GDPRComplianceManager {
  constructor(HaypbooksClient) {
    this.Haypbooks = HaypbooksClient;
  ...
  async findSubjectData(subjectId, subjectType) {
    ...
    try {
      ...
      if (subjectType === 'customer') {
        const customerResponse = await this.Haypbooks.makeAPIRequest(`customer/${subjectId}`);
  ...
  async findRelatedTransactions(subjectId) {
    ...
    // Find invoices
    const invoiceResponse = await this.Haypbooks.makeAPIRequest(`invoice?where=CustomerRef.value='${subjectId}'`);
    ...
    // Find payments
    const paymentResponse = await this.Haypbooks.makeAPIRequest(`payment?where=CustomerRef.value='${subjectId}'`);
  ...
  async findRelatedAttachments(subjectId) {
    ...
    const attachResponse = await this.Haypbooks.makeAPIRequest(`attachable?where=AttachableRef.EntityRef.type='Customer' AND AttachableRef.EntityRef.value='${subjectId}'`);
  ...
  async deleteOrAnonymizeData(item) {
    try {
      switch (item.type) {
        ...
        case 'Payment':
          // Delete transaction
          await this.Haypbooks.makeAPIRequest(`${item.type.toLowerCase()}/${item.id}`, {
            method: 'POST',
            body: JSON.stringify({
              Id: item.id,
        ...
        case 'Attachment':
          // Delete attachment
          await this.Haypbooks.makeAPIRequest(`attachable/${item.id}`, {
            method: 'DELETE'
          });
          break;
      }
  ...
  async anonymizeCustomer(customerId) {
    ...
    await this.Haypbooks.makeAPIRequest(`customer/${customerId}`, {
      method: 'POST',
  ...
  async findOldData(entityType, cutoffDate) {
    const response = await this.Haypbooks.makeAPIRequest(`${entityType}?where=MetaData.LastUpdatedTime < '${cutoffDate.toISOString()}'`);
...
// Usage
const gdprManager = new GDPRComplianceManager(HaypbooksClient);
...
const response = await connectionPool.executeRequest(
  'https://Haypbooks.api.intuit.com/v3/company/123/customer',
  {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${accessToken}`,
...
This completes the comprehensive Haypbooks Online Developer Guide with advanced API integration patterns, enterprise features, security implementations, and performance optimization strategies. The guide provides developers with everything needed to build robust, scalable integrations with Haypbooks Online. 

---

*For basic user operations and getting started, see the Haypbooks User Guide. For advanced administration and management, see the Haypbooks Admin Guide. For quick reference materials, see the Haypbooks Reference Guide.*
