...
- `wait-for-backend.js` — wait for the Backend health endpoint to become healthy before running E2E tests (moved to `Haypbooks/Backend/scripts`).
- `tmp_check_table.js` — temporary helper used for local DB checks (moved here).

Usage examples:

- Run E2E: (from Frontend package)
  npm run e2e -- (this will call `Haypbooks/Backend/scripts/wait-for-backend.js` first)
